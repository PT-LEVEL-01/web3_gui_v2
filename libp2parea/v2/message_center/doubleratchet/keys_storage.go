...
type KeysStorage interface {
	// get按给定的键和消息编号返回消息键。
	Get(k keystore.Key, msgNum uint) (mk keystore.Key, ok bool, err error)

	// 将给定的mk保存在指定的键和msgnum下。
	Put(sessionID []byte, k keystore.Key, msgNum uint, mk keystore.Key, keySeqNum uint) error

	// DeleteMk ensures there's no message key under the specified key and msgNum.
	//deletemk确保在指定的密钥和msgnum下没有消息密钥。
	DeleteMk(k keystore.Key, msgNum uint) error
	...
	Count(k keystore.Key) (uint, error)

	// 全部返回所有键
	All() (map[keystore.Key]map[uint]keystore.Key, error)
}
...
// get按给定的键和消息编号返回消息键。
func (this *KeysStorageInMemory) Get(pubKey keystore.Key, msgNum uint) (keystore.Key, bool, error) {
	if this.keys == nil {
...
type InMemoryKey struct {
	messageKey keystore.Key
	seqNum     uint
	sessionID  []byte
}

// 将给定的mk保存在指定的键和msgnum下。
func (s *KeysStorageInMemory) Put(sessionID []byte, pubKey keystore.Key, msgNum uint, mk keystore.Key, seqNum uint) error {
	if s.keys == nil {
		s.keys = new(sync.Map)
...
// deletemk确保在指定的密钥和msgnum下没有消息密钥。
func (s *KeysStorageInMemory) DeleteMk(pubKey keystore.Key, msgNum uint) error {
	if s.keys == nil {
		return nil
	}
	// if _, ok := s.keys[pubKey]; !ok {
...
// count返回存储在指定密钥下的消息密钥数。
func (s *KeysStorageInMemory) Count(pubKey keystore.Key) (uint, error) {
	if s.keys == nil {
		return 0, nil
	}
...
// 全部返回所有键
func (s *KeysStorageInMemory) All() (map[keystore.Key]map[uint]keystore.Key, error) {
	response := make(map[keystore.Key]map[uint]keystore.Key)
	...
	s.keys.Range(func(k, v interface{}) bool {
		pubKey := k.(keystore.Key)
		response[pubKey] = make(map[uint]keystore.Key)
		keys := v.(*sync.Map)
		keys.Range(func(kj, vj interface{}) bool {
			n, ok := kj.(uint)
			if !ok {
