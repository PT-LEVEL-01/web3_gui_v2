...
func TestExample(t *testing.T) {
	...
	alice, err := NewWithRemoteKey([]byte("alice-session-id"), sk, keyPair.GetPublicKey(), nil)
	if err != nil {
		log.Fatal(err)
	}
...
func TestExample2(t *testing.T) {
	...
	bob, _ := NewWithRemoteKey([]byte("bob-session-id"), sk, bobKeyPair.GetPublicKey(), nil)
...
func TestExample3(t *testing.T) {
	...
	alice, _ := NewHEWithRemoteKey(sk, sharedHka, sharedNhkb, keyPair.GetPublicKey())

	m := alice.RatchetEncrypt([]byte("Hi Bob!"), nil)
	fmt.Println("加密头", hex.EncodeToString(m.Header))
	fmt.Println("加密内容", hex.EncodeToString(m.Ciphertext))
	plaintext, _ := bob.RatchetDecrypt(m, nil)
	fmt.Println(string(plaintext))

	m = alice.RatchetEncrypt([]byte("Hi Bob 2!"), nil)
	fmt.Println("加密头", hex.EncodeToString(m.Header))
	fmt.Println("加密内容", hex.EncodeToString(m.Ciphertext))
	plaintext, _ = bob.RatchetDecrypt(m, nil)
	fmt.Println(string(plaintext))
