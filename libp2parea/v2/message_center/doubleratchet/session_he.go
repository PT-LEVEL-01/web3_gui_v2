...
func NewHE(sharedKey, sharedHka, sharedNhkb keystore.Key, keyPair DHPair, opts ...option) (SessionHE, error) {
	...
	return &sessionHE{state, new(sync.Mutex)}, nil
}

// NewHEWithRemoteKey creates session with the shared keys and public key of the other party.
func NewHEWithRemoteKey(sharedKey, sharedHka, sharedNhkb, remoteKey keystore.Key, opts ...option) (SessionHE, error) {
	state, err := newState(sharedKey, opts...)
	if err != nil {
		return nil, err
	}
...
func (s *sessionHE) RatchetDecrypt(m MessageHE, ad []byte) ([]byte, error) {
	s.lock.Lock()
	defer s.lock.Unlock()
	//utils.Log.Info().Msgf("检查是否跳过:%s %s", hex.EncodeToString(m.Header), hex.EncodeToString(m.Ciphertext))
	// Is the message one of the skipped?
	if plaintext, err := s.trySkippedMessages(m, ad); err != nil || plaintext != nil {
		return plaintext, err
	}
	//utils.Log.Info().Msgf("检查是否跳过")
	...
	//utils.Log.Info().Msgf("检查是否跳过:%s %d", hex.EncodeToString(sc.HKr[:]), h.N)
