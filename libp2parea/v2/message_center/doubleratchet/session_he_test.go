...
var (
	sharedHka  = keystore.Key{0xbd, 0x29, 0x18, 0xcb, 0x18, 0x6c, 0x26, 0x32, 0xd5, 0x82, 0x41, 0x2d, 0x11, 0xa4, 0x55, 0x87, 0x1e, 0x5b, 0xa3, 0xb5, 0x5a, 0x6d, 0xe1, 0x97, 0xde, 0xf7, 0x5e, 0xc3, 0xf2, 0xec, 0x1d, 0xd}
	sharedNhkb = keystore.Key{0x32, 0x89, 0x3a, 0xed, 0x4b, 0xf0, 0xbf, 0xc1, 0xa5, 0xa9, 0x53, 0x73, 0x5b, 0xf9, 0x76, 0xce, 0x70, 0x8e, 0xe1, 0xa, 0xed, 0x98, 0x1d, 0xe3, 0xb4, 0xe9, 0xa9, 0x88, 0x54, 0x94, 0xaf, 0x23}
)
...
func TestNewHE(t *testing.T) {
	...
	// Assert.
	require.Nil(t, err)
	require.Equal(t, keystore.NewDHPair(bobPair.GetPrivateKey(), bobPair.GetPublicKey()), s.DHs)
...
func TestNewHEWithRemoteKey(t *testing.T) {
	// Act.
	var (
		si, err = NewHEWithRemoteKey(sk, sharedHka, sharedNhkb, bobPair.GetPublicKey())
		s       = si.(*sessionHE)
	)

	// Assert.
	require.Nil(t, err)
	require.NotEqual(t, keystore.DHPair{}, s.DHs)
	require.Equal(t, bobPair.GetPublicKey(), s.DHr)
	require.NotEqual(t, keystore.Key{}, s.NHKs)
	require.Equal(t, sharedHka, s.HKs)
	require.Equal(t, sharedNhkb, s.NHKr)
	require.Equal(t, sharedHka, s.HKs)
	require.NotEqual(t, keystore.Key{}, s.RootCh.CK)
	require.NotEqual(t, sk, s.RootCh.CK)
	require.NotEqual(t, keystore.Key{}, s.SendCh.CK)
...
func TestSessionHE_RatchetDecrypt_CommunicationAliceSends(t *testing.T) {
	...
	var (
		...
		alice, _ = NewHEWithRemoteKey(sk, sharedHka, sharedNhkb, bobPair.GetPublicKey())
	)
...
func TestSessionHE_RatchetDecrypt_CommunicationSkippedMessages(t *testing.T) {
	...
	var (
		...
		aliceI, _ = NewHEWithRemoteKey(sk, sharedHka, sharedNhkb, bobPair.GetPublicKey(), WithMaxSkip(1))
...
func TestSessionHE_OldKeysDeletion(t *testing.T) {
	...
	var (
		...
		alice, _ = NewHEWithRemoteKey(sk, sharedHka, sharedNhkb, bobPair.GetPublicKey(), WithMaxKeep(2))
	)
...
func TestSessionHE_ExtraKeysDeletion(t *testing.T) {
	...
	var (
		...
		alice, _ = NewHEWithRemoteKey(sk, sharedHka, sharedNhkb, bobPair.GetPublicKey(), WithMaxMessageKeysPerSession(2))
	)
