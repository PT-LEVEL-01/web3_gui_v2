...
func (c DefaultCrypto) GenerateDH() (DHPair, error) {
	...
	if _, err := io.ReadFull(rand.Reader, privKey[:]); err != nil {
		return keystore.DHPair{}, fmt.Errorf("couldn't generate privKey: %s", err)
	}
	privKey[0] &= 248
	privKey[31] &= 127
	privKey[31] |= 64
	...
	return keystore.NewDHPair(privKey, pubKey), nil
}

// 计算公共密钥
func (c DefaultCrypto) DH(dhPair DHPair, dhPub keystore.Key) keystore.Key {
	var (
		dhOut   [32]byte
...
func (c DefaultCrypto) KdfRK(rk, dhOut keystore.Key) (rootKey, chainKey, headerKey keystore.Key) {
	var (
...
func (c DefaultCrypto) KdfCK(ck keystore.Key) (chainKey keystore.Key, msgKey keystore.Key) {
	const (
		ckInput = 15
		mkInput = 16
	)
...
func (this DefaultCrypto) Encrypt(mk keystore.Key, plaintext, ad []byte) []byte {
...
func (c DefaultCrypto) Decrypt(mk keystore.Key, authCiphertext, ad []byte) ([]byte, error) {
	var (
...
func (c DefaultCrypto) deriveEncKeys(mk keystore.Key) (encKey keystore.Key, authKey keystore.Key, iv [16]byte) {
	// 首先，从mk导出加密和身份验证密钥。
	salt := make([]byte, 32)
	var (
