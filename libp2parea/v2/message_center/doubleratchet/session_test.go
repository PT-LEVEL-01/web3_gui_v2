...
func TestNew(t *testing.T) {
	...
	// Assert.
	require.Nil(t, err)
	require.NotEqual(t, keystore.Key{}, s.DHs.GetPrivateKey())
	require.NotEqual(t, keystore.Key{}, s.DHs.GetPublicKey())
}
...
func TestNewWithRemoteKey(t *testing.T) {
	// Act.
	si, err := NewWithRemoteKey([]byte("id"), sk, bobPair.GetPublicKey(), nil)
...
func TestNewWithRemoteKey_BadOption(t *testing.T) {
	// Act.
	_, err := NewWithRemoteKey([]byte("id"), sk, bobPair.GetPublicKey(), nil, WithMaxSkip(-10))
...
func TestSession_RatchetDecrypt_CommunicationAliceSends(t *testing.T) {
	...
	var (
		bob, _   = New([]byte("bob"), sk, bobPair, nil)
		alice, _ = NewWithRemoteKey([]byte("alice"), sk, bobPair.GetPublicKey(), nil)
	)
...
func TestSession_RatchetDecrypt_CommunicationSkippedMessages(t *testing.T) {
	...
	var (
		...
		aliceI, _ = NewWithRemoteKey([]byte("alice"), sk, bob.DHs.GetPublicKey(), nil, WithMaxSkip(1))
...
func TestSession_SkippedKeysDeletion(t *testing.T) {
	...
	var (
		...
		alice, _ = NewWithRemoteKey([]byte("alice"), sk, bobPair.GetPublicKey(), nil, WithMaxKeep(2))
		h        = SessionTestHelper{t, alice, bob}
	)
