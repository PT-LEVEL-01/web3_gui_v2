...
type Session interface {
	...
	DeleteMk(keystore.Key, uint32) error
}
...
// 新建使用共享密钥创建会话。
func New(id []byte, sharedKey keystore.Key, keyPair DHPair, storage SessionStorage, opts ...option) (Session, error) {
...
func NewWithRemoteKey(id []byte, sharedKey, remoteKey keystore.Key, storage SessionStorage, opts ...option) (Session, error) {
	state, err := newState(sharedKey, opts...)
	if err != nil {
		return nil, err
	}
