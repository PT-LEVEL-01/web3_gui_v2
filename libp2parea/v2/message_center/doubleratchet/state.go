...
type State struct {
	...
	HKr, NHKr keystore.Key

	//发送头部key和下一个头部key，仅用于头部加密。
	HKs, NHKs keystore.Key
...
func DefaultState(sharedKey keystore.Key) State {
	c := DefaultCrypto{}
...
func newState(sharedKey keystore.Key, opts ...option) (State, error) {
...
func (s *State) skipMessageKeys(key keystore.Key, until uint) ([]skippedKey, error) {
