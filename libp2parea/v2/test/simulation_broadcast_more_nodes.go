...
func BuildIdss() []*NodeOne {
	...
	for n := 0; n < len(ids); n++ {
		...
		for _, one := range is {
			...
			idOne, ERR := nodeStore.BuildAddrByData(nodeOne.Self.GetPre(), one)
			if ERR.CheckFail() {
...
// 		idsm := nodeStore.NewKademlia(NodeOne.Self, gconfig.NodeIDLevel)
// 		for i, _ := range nodes {
// 			if i == index {
// 				continue
// 			}
...
func BuildIdsTree() []*Node {
	...
	for i := 0; i < 10; i++ {
		pukBs := []byte(base + strconv.Itoa(i))
		addrOne, ERR := nodeStore.BuildAddr("", pukBs)
		nodeOne := &Node{
			Self:        *addrOne,
			Nodes:       make([]*Node, 0),
	...
	for index, one := range nodes {
		idsm := nodeStore.NewKademlia(one.Self, 256)
		for i, _ := range nodes {
			if i == index {
				continue
			}
...
func SimulateMoreNodes() []*NodeOne {
	...
	for n := 0; n < len(ids); n++ {
		...
		// idMH := nodeStore.AddressFromB58String(ids[index])
		idMH := ids[n]
		idsm := nodeStore.NewKademlia(idMH, 256)
		nodeOne.Self = idMH
		for i, _ := range ids {
			if i == index {
				continue
