...
func BuildIds() []*NodeOne {
	...
	for n := 0; n < len(ids); n++ {
		...
		idsm := nodeStore.NewKademlia(idMH.Data(), 256, &utils.Log)
		nodeOne.Self = idMH
		for i, one := range ids {
			if i == index {
				continue
		...
		is := idsm.GetIds()
		for _, one := range is {
			idOne, ERR := nodeStore.BuildAddrByData(nodeOne.Self.GetPre(), one) // nodeStore.AddressNet(one)
			if ERR.CheckFail() {
				panic(ERR.String())
			}
...
func BuildIdsTwo() []*NodeOne {
	...
	for n := 0; n < len(ids); n++ {
		...
		// idsm := nodeStore.NewKademlia(idMH, gconfig.NodeIDLevel)
		nodeOne.Self = idMH
		nodes = append(nodes, nodeOne)
	}
...
func broadcast(nodes []*NodeOne) {
	for i, one := range nodes {
		...
		for j, _ := range one.Nodes {
			...
			for x, _ := range nodes {
				temp := nodes[x]
				if bytes.Equal(tree.Data(), temp.Self.Data()) {
...
func (this *NodeOne) GetIdsForFar(id nodeStore.AddressNet) []nodeStore.AddressNet {
	...
	kl.Add(new(big.Int).SetBytes(this.Self.Data()))
	kl.Add(new(big.Int).SetBytes(id.Data()))

	for i, _ := range this.Nodes {
		kl.Add(new(big.Int).SetBytes(this.Nodes[i].Data()))
	}
	...
	for _, one := range list {
		...
		if bytes.Equal(one.Bytes(), this.Self.Data()) {
			find = true
		} else {
			if find {
				...
				mh, ERR := nodeStore.BuildAddrByData(id.GetPre(), one.Bytes())
				if ERR.CheckFail() {
					panic(ERR.String())
				}
...
func BuildIdsTrees() []*Node {
	...
	for n := 0; n < len(ids); n++ {
		...
		idBuilder := nodeStore.NewLogicNumBuider(nodeOne.Self.Data(), 256)
...
func BuildIds4() []*Node {
	...
	for n := 0; n < len(ids); n++ {
		...
		idBuilder := nodeStore.NewLogicNumBuider(nodeOne.Self, 256)
		nodeOne.LogicIds = idBuilder.GetNodeNetworkNum()
		nodes = append(nodes, nodeOne)
	}
	...
	for n := 0; n < 10; n++ {
		...
		for _, nodeOne := range nodes {
			...
			for _, logicNodeOne := range nodeOne.Nodes {
				idsm := nodeStore.NewKademlia(nodeOne.Self, nodeStore.NodeIdLevel)
...
func (this *Node) AddNode(node *Node) {
	...
	idm := nodeStore.NewKademlia(this.Self, nodeStore.NodeIdLevel)
	// ids := GetLogicNodes()
	for _, one := range this.Nodes {
		idm.AddId(one.Self)
	}
