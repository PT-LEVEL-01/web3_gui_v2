...
// 	utils.Log.Debug().Msgf("开始启动局域网广播服务器")
...
// 		addr, err = net.ResolveUDPAddr("udp", "0.0.0.0:"+strconv.Itoa(broadcastServerPort+i))
// 		if err != nil {
// 			// log.Panic(err)
// 			continue
// 		}
...
// 		utils.Log.Debug().Msgf("局域网广播客户端正在运行")
// 		return
// 	}
// 	utils.Log.Debug().Msgf("正在启动局域网广播客户端")
...
// 			continue
// 		}
// 		utils.Log.Debug().Msgf("局域网广播客户端启动成功，监听端口：%s", conn.LocalAddr().String())
...
// 	//启动失败
// 	if conn == nil {
// 		utils.Log.Debug().Msgf("启动局域网广播客户端失败")
// 		return
// 	}
// 	// if len(conns) == 0 {
// 	// 	utils.Log.Debug().Msgf("启动局域网广播客户端失败")
...
// 		//		AddSubscribe(c)
// 		<-c
// 		utils.Log.Debug().Msgf("开始关闭局域网广播客户端")
// 		broadcastClientIsStart = false
// 		conn.Close()
