package message_center

import (
	"bytes"
	"encoding/binary"
	"errors"
	"github.com/gogo/protobuf/proto"
	"strconv"
	"strings"
	"sync"
	"time"
	"web3_gui/keystore/v1/crypto"
	"web3_gui/keystore/v1/crypto/dh"
	"web3_gui/libp2parea/v1/config"
	"web3_gui/libp2parea/v1/engine"
	"web3_gui/libp2parea/v1/message_center/flood"
	"web3_gui/libp2parea/v1/nodeStore"
	"web3_gui/libp2parea/v1/protobuf/go_protobuf"
	"web3_gui/libp2parea/v1/virtual_node"
	"web3_gui/utils"
)

/*
注册邻居节点消息，不转发
*/
func (this *MessageCenter) Register_neighbor(msgid uint64, handler MsgHandler, isSysMsg bool) {
	this.router.Register(msgid, handler, isSysMsg)
}

/*
发送一个新邻居节点消息
发出去不管
@return    bool    消息是否发送成功
*/
func (this *MessageCenter) SendNeighborMsg(msgid uint64, recvid *nodeStore.AddressNet, content *[]byte) (*Message, error) {
	if !this.CheckOnline() {
		return nil, config.ERROR_offline
	}
	if recvid == nil || len(*recvid) == 0 {
		return nil, config.ERROR_params_fail
	}
	session, ok := this.sessionEngine.GetSession(this.areaName, utils.Bytes2string(*recvid))
	if !ok {
		return nil, config.ERROR_get_node_conn_fail
	}
	return this.SendNeighborMsgBySession(msgid, session, recvid, content)
}

/*
发送一个新邻居节点消息
发出去不管
@return    bool    消息是否发送成功
*/
func (this *MessageCenter) SendNeighborMsgBySession(msgid uint64, session engine.Session, recvid *nodeStore.AddressNet, content *[]byte) (*Message, error) {
	if !this.CheckOnline() {
		return nil, config.ERROR_offline
	}
	//直接查找最近的超级节点
	mhead := NewMessageHead(this.nodeManager.NodeSelf, this.nodeManager.GetSuperPeerId(), recvid, recvid, false, this.nodeManager.GetMachineID(), session.GetMachineID())
	mbody := NewMessageBody(msgid, content, 0, nil, 0)
	message := NewMessage(mhead, mbody)
	message.BuildHash()
	// session, ok := engine.GetSession(recvid.B58String())
	session, ok := this.sessionEngine.GetSession(this.areaName, utils.Bytes2string(*recvid))
	if ok {
		// fmt.Println("给这个session发送消息", recvid.B58String())
		// err := session.Send(version_neighbor, mhead.JSON(), mbody.JSON(), false)
		mheadBs := mhead.Proto()
		mbodyBs, err := mbody.Proto()
		if err != nil {
			return nil, err
		}

		err = session.Send(version_neighbor, &mheadBs, &mbodyBs, 0)
		if err != nil {
			if strings.Contains(err.Error(), "use of closed network connection") {
			} else if strings.Contains(err.Error(), "An existing connection was forcibly closed by the remote host") {
			} else if strings.Contains(err.Error(), "An established connection was aborted by the software in your host machine") {
			} else if err.Error() == engine.ERROR_send_cache_close.Error() {
			} else {
				utils.Log.Error().Msgf("msg send error:%s", err.Error())
			}
			return message, err
		}
		return message, nil
	} else {
		// utils.Log.Error().Msgf("get node conn fail %s", recvid.B58String())
		// nodeStore.DelNode(recvid)
		return message, config.ERROR_get_node_conn_fail
	}
}

/*
对某个消息回复
*/
func (this *MessageCenter) SendNeighborReplyMsg(message *Message, msgid uint64, content *[]byte, session engine.Session) error {
	if !this.CheckOnline() {
		return config.ERROR_offline
	}
	// goroutineId := utils.GetRandomDomain() + utils.TimeFormatToNanosecondStr()
	// _, file, line, _ := runtime.Caller(0)
	// engine.AddRuntime(file, line, goroutineId)
	// defer engine.DelRuntime(file, line, goroutineId)

	head := NewMessageHeadProxy(this.nodeManager.NodeSelf, this.nodeManager.GetSuperPeerId(), message.Head.Sender, message.Head.SenderSuperId, message.Head.SenderProxyId, nil, true, this.nodeManager.GetMachineID(), message.Head.SenderMachineID)
	// utils.Log.Info().Msgf("看看是哪个指针为空 %v", message.Body)
	body := NewMessageBody(msgid, content, message.Body.CreateTime, message.Body.Hash, message.Body.SendRand)
	newmessage := NewMessage(head, body)
	newmessage.BuildReplyHash(message.Body.CreateTime, message.Body.Hash, message.Body.SendRand)
	// return session.Send(version_neighbor, head.JSON(), body.JSON(), false)
	mheadBs := head.Proto()
	mbodyBs, err := body.Proto()
	if err != nil {
		return err
	}
	if session == nil {
		return errors.New("session is nil")
	}
	return session.Send(version_neighbor, &mheadBs, &mbodyBs, 0)
}

// func (this *MessageCenter) SendNeighborWithReplyMsg(msgid uint64, recvid *nodeStore.AddressNet, content *[]byte,
// 	waitRequestClass string, timeout time.Duration) (*[]byte, error) {

// 	if !this.nodeManager.CheckOnline() {
// 		return nil, config.ERROR_offline
// 	}
// 	message, err := this.SendNeighborMsg(msgid, recvid, content)
// 	if err != nil {
// 		return nil, err
// 	}
// 	bs, err := flood.WaitRequest(waitRequestClass, utils.Bytes2string(message.Body.Hash), timeout)
// 	// bs, err := flood.WaitResponse(utils.Bytes2string(message.Body.Hash), timeout)
// 	if err != nil {
// 		return nil, err
// 	}
// 	return bs, nil
// }

/*
发送一个新邻居节点消息
发送出去，等返回
*/
func (this *MessageCenter) SendNeighborMsgWaitRequest(msgid uint64, recvid *nodeStore.AddressNet,
	content *[]byte, timeout time.Duration) (*[]byte, error) {
	if !this.CheckOnline() {
		return nil, config.ERROR_offline
	}
	if recvid == nil || len(*recvid) == 0 {
		return nil, config.ERROR_params_fail
	}
	//直接查找最近的超级节点
	mhead := NewMessageHead(this.nodeManager.NodeSelf, this.nodeManager.GetSuperPeerId(), recvid, recvid, false, this.nodeManager.GetMachineID(), "")
	mbody := NewMessageBody(msgid, content, 0, nil, 0)
	message := NewMessage(mhead, mbody)
	message.BuildHash()

	// flood.GroupWaitRecv.RegisterRequest(utils.Bytes2string(*recvid), utils.Bytes2string(message.Body.Hash))
	// defer flood.GroupWaitRecv.RemoveRequest(utils.Bytes2string(*recvid), utils.Bytes2string(message.Body.Hash))
	// flood.RegisterRequest(utils.Bytes2string(message.Body.Hash))
	// defer flood.RemoveRequest(utils.Bytes2string(message.Body.Hash))
	// session, ok := engine.GetSession(recvid.B58String())
	// utils.Log.Info().Msgf("recvid:%s", recvid.B58String())
	session, ok := this.sessionEngine.GetSession(this.areaName, utils.Bytes2string(*recvid))
	if ok {
		flood.GroupWaitRecv.RegisterRequest(strconv.Itoa(int(session.GetIndex())), utils.Bytes2string(message.Body.Hash))
		defer flood.GroupWaitRecv.RemoveRequest(strconv.Itoa(int(session.GetIndex())), utils.Bytes2string(message.Body.Hash))
		// err := session.Send(version_neighbor, mhead.JSON(), mbody.JSON(), false)
		mheadBs := mhead.Proto()
		mbodyBs, err := mbody.Proto()
		if err != nil {
			return nil, err
		}
		err = session.Send(version_neighbor, &mheadBs, &mbodyBs, timeout)
		if err != nil {
			utils.Log.Error().Msgf("msg send error:%s self:%s target:%s", err.Error(), this.nodeManager.NodeSelf.IdInfo.Id.B58String(), recvid.B58String())
			return nil, err
		}
		// return message, nil
		return flood.GroupWaitRecv.WaitResponse(strconv.Itoa(int(session.GetIndex())), utils.Bytes2string(message.Body.Hash), timeout)
		// return flood.WaitResponse(utils.Bytes2string(message.Body.Hash), timeout)
	} else {
		// utils.Log.Error().Msgf("get node conn fail:%s", recvid.B58String())
		// nodeStore.DelNode(recvid)
		return nil, config.ERROR_get_node_conn_fail
	}
}

/*
注册广播消息
*/
func (this *MessageCenter) Register_multicast(msgid uint64, handler MsgHandler, isSysMsg bool) {
	this.router.Register(msgid, handler, isSysMsg)
}

/*
	发送一个新的广播消息
*/
// func SendMulticastMsg(msgid uint64, content *[]byte) bool {
// 	//TODO 这里查询缓存中的节点信息，拿到节点的超级节点信息
// 	head := NewMessageHead(nil, nil, false)
// 	body := NewMessageBody(msgid, content, "", nil, 0)
// 	message := NewMessage(head, body)
// 	message.BuildHash()

// 	failNode := append(nodeStore.GetLogicNodes(), nodeStore.GetProxyAll()...)
// 	failNode = append(failNode, nodeStore.GetOtherNodes()...)
// 	// engine.GetAllSession()
// 	//排除重复的地址
// 	failNode = nodeStore.RemoveDuplicateAddress(failNode)

// 	broadcasts := make([]*nodeStore.AddressNet, 0)
// 	//广播失败的节点最多广播5次
// 	for i := 0; i < 1; i++ {
// 		if len(failNode) <= 0 {
// 			break
// 		}
// 		// utils.Log.Info().Msgf("第 %d 轮发送广播", i)
// 		broadcasts = failNode
// 		failNode = make([]*nodeStore.AddressNet, 0)

// 		for j, _ := range broadcasts {
// 			//区块广播给节点
// 			// utils.Log.Warn().Msgf("广播给节点 %d %s", j, addrStr)
// 			// if ss, ok := engine.GetSession(broadcasts[j].B58String()); ok {
// 			if ss, ok := engine.GetSession(utils.Bytes2string(*broadcasts[j])); ok {
// 				err := ss.Send(version_multicast, head.JSON(), body.JSON(), false)
// 				if err != nil {
// 					utils.Log.Warn().Msgf("send multicast message fail %s", err.Error()) // fmt.Println("发送广播消息失败", err)
// 					continue
// 				} else {
// 					// hashStr := hex.EncodeToString(message.Body.Hash)
// 					// utils.Log.Warn().Msgf("广播消息 1111111111111 %s", hashStr)
// 					// bs := flood.WaitRequest(config.CLASS_wallet_broadcast_return, hex.EncodeToString(message.Body.Hash), int64(i+1))
// 					bs := flood.WaitRequest(config.CLASS_wallet_broadcast_return, utils.Bytes2string(message.Body.Hash), int64(i+1))
// 					if bs == nil {
// 						utils.Log.Warn().Msgf("Timeout receiving broadcast reply message %s %s", broadcasts[j].B58String(), hex.EncodeToString(message.Body.Hash))
// 						failNode = append(failNode, broadcasts[j])
// 						continue
// 					}
// 					// utils.Log.Warn().Msgf("收到广播回复消息  %s %s", addrStr, hashStr)
// 				}
// 			} else {
// 				utils.Log.Warn().Msgf("get node conn fail")
// 			}

// 		}
// 	}
// 	return true
// }

/*
发送一个新的广播消息
*/
func (this *MessageCenter) SendMulticastMsg(msgid uint64, content *[]byte) error {
	// utils.Log.Info().Msgf("发送广播 start")
	//TODO 这里查询缓存中的节点信息，拿到节点的超级节点信息
	head := NewMessageHead(this.nodeManager.NodeSelf, this.nodeManager.GetSuperPeerId(), nil, nil, false, this.nodeManager.GetMachineID(), "")
	body := NewMessageBody(msgid, content, 0, nil, 0)
	message := NewMessage(head, body)
	message.BuildHash()
	bodyBs, err := body.Proto()
	if err != nil {
		return err
	}
	//先保存这个消息到缓存
	// err = new(sqlite3_db.MessageCache).Add(message.Body.Hash, head.Proto(), bodyBs)
	// if err != nil {
	// 	utils.Log.Error().Msgf(err.Error())
	// 	return nil
	// }

	//先保存这个消息到缓存
	headBs := head.Proto()
	err = AddMessageCacheByBytes(&message.Body.Hash, &headBs, &bodyBs, this.levelDB)
	if err != nil {
		utils.Log.Error().Msgf(err.Error())
		return err
	}

	superNodes := this.nodeManager.GetLogicNodes()
	superNodes = append(superNodes, this.nodeManager.GetNodesClient()...)
	//再发送给内网节点
	proxyNodes := this.nodeManager.GetProxyAll()
	err = this.BroadcastsAll(version_multicast, 0, nil, superNodes, proxyNodes, &message.Body.Hash)
	return err
}

/*
发送一个新的广播消息
*/
func (this *MessageCenter) SendMulticastMsgWaitRequest(msgid uint64, content *[]byte, timeout time.Duration) (interface{}, error) {
	// utils.Log.Info().Msgf("发送广播 start")
	//TODO 这里查询缓存中的节点信息，拿到节点的超级节点信息
	head := NewMessageHead(this.nodeManager.NodeSelf, this.nodeManager.GetSuperPeerId(), nil, nil, false, this.nodeManager.GetMachineID(), "")
	body := NewMessageBody(msgid, content, 0, nil, 0)
	message := NewMessage(head, body)
	message.BuildHash()
	bodyBs, err := body.Proto()
	if err != nil {
		return nil, err
	}
	//先保存这个消息到缓存
	// err = new(sqlite3_db.MessageCache).Add(message.Body.Hash, head.Proto(), bodyBs)
	// if err != nil {
	// 	utils.Log.Error().Msgf(err.Error())
	// 	return nil
	// }

	//先保存这个消息到缓存
	headBs := head.Proto()
	err = AddMessageCacheByBytes(&message.Body.Hash, &headBs, &bodyBs, this.levelDB)
	if err != nil {
		utils.Log.Error().Msgf(err.Error())
		return nil, err
	}

	superNodes := this.nodeManager.GetLogicNodes()
	superNodes = append(superNodes, this.nodeManager.GetNodesClient()...)
	//再发送给内网节点
	proxyNodes := this.nodeManager.GetProxyAll()
	flood.RegisterRequest(utils.Bytes2string(message.Body.Hash))
	defer flood.RemoveRequest(utils.Bytes2string(message.Body.Hash))
	err = this.BroadcastsAll(version_multicast, 0, nil, superNodes, proxyNodes, &message.Body.Hash)
	if err != nil {
		return nil, err
	}
	return flood.WaitResponseItr(utils.Bytes2string(message.Body.Hash), timeout)
}

func (this *MessageCenter) BroadcastsAll(msgid, p2pMsgid uint64, whiltlistNodes, superNodes,
	proxyNodes []nodeStore.AddressNet, hash *[]byte) error {

	// start := time.Now()
	//给已发送的节点放map里，避免重复发送
	allNodes := make(map[string]bool)

	var timeouterrorlock = new(sync.Mutex)
	var timeouterror error

	//先发送给超级节点
	// superNodes := nodeStore.GetLogicNodes()
	//排除重复的地址
	// superNodes = nodeStore.RemoveDuplicateAddress(superNodes)
	cs := make(chan bool, config.CPUNUM)
	group := new(sync.WaitGroup)
	for i, _ := range whiltlistNodes {
		sessionid := whiltlistNodes[i]
		//不要发送给自己
		if bytes.Equal(this.nodeManager.NodeSelf.IdInfo.Id, sessionid) {
			continue
		}
		_, ok := allNodes[utils.Bytes2string(sessionid)]
		if ok {
			// utils.Log.Info().Msgf("repeat node addr: %s", sessionid.B58String())
			continue
		}
		allNodes[utils.Bytes2string(sessionid)] = false
		cs <- false
		group.Add(1)
		//区块广播给节点
		// utils.Log.Info().Msgf("multcast super node:%s", sessionid.B58String())
		utils.Go(func() {
			ok := this.broadcastsOne(msgid, p2pMsgid, cs, sessionid, hash, group, true, 0)
			if ok {
				return
			}
			timeouterrorlock.Lock()
			timeouterror = config.ERROR_wait_msg_timeout
			timeouterrorlock.Lock()
		}, nil)
	}
	group.Wait()
	// utils.Log.Info().Msgf("multicast whilt list node time %s", time.Now().Sub(start))

	// start = time.Now()
	for i, _ := range superNodes {
		sessionid := superNodes[i]
		//不要发送给自己
		if bytes.Equal(this.nodeManager.NodeSelf.IdInfo.Id, sessionid) {
			continue
		}
		_, ok := allNodes[utils.Bytes2string(sessionid)]
		if ok {
			// utils.Log.Info().Msgf("repeat node addr: %s", sessionid.B58String())
			continue
		}
		allNodes[utils.Bytes2string(sessionid)] = false
		cs <- false
		group.Add(1)
		//区块广播给节点
		// utils.Log.Info().Msgf("multcast super node:%s", sessionid.B58String())
		go this.broadcastsOne(msgid, p2pMsgid, cs, sessionid, hash, group, false, 0)
	}
	group.Wait()
	// utils.Log.Info().Msgf("multicast super node time %s", time.Now().Sub(start))
	// start = time.Now()

	//再发送给内网节点
	// proxyNodes := nodeStore.GetProxyAll()
	//排除重复的地址
	// proxyNodes = nodeStore.RemoveDuplicateAddress(proxyNodes)
	for i, _ := range proxyNodes {
		sessionid := proxyNodes[i]
		//不要发送给自己
		if bytes.Equal(this.nodeManager.NodeSelf.IdInfo.Id, sessionid) {
			continue
		}
		_, ok := allNodes[utils.Bytes2string(sessionid)]
		if ok {
			// utils.Log.Info().Msgf("repeat node addr: %s", sessionid.B58String())
			continue
		}
		allNodes[utils.Bytes2string(sessionid)] = false
		cs <- false
		group.Add(1)
		//区块广播给节点
		go this.broadcastsOne(msgid, p2pMsgid, cs, sessionid, hash, group, false, 0)
	}
	group.Wait()
	// utils.Log.Info().Msgf("multicast proxy node time %s", time.Now().Sub(start))
	return timeouterror
}

/*
给一个节点发送广播消息
@return    bool    白名单种节点是否都收到了消息
*/
func (this *MessageCenter) broadcastsOne(msgid, p2pMsgid uint64, cs chan bool, sid nodeStore.AddressNet,
	hash *[]byte, group *sync.WaitGroup, reply bool, timeout time.Duration) (success bool) {
	// utils.Log.Info().Msgf("multicast to %s", sid.B58String())
	//先查询对方的版本,看协议是否有升级
	// node := nodeStore.FindNode(&sid)
	// if node == nil {
	// 	<-cs
	// 	group.Done()
	// 	return
	// }
	// if nodeStore.NodeSelf.Version >= config.Version_1 && node.Version >= config.Version_1 {

	// }
	head := NewMessageHead(this.nodeManager.NodeSelf, this.nodeManager.GetSuperPeerId(), nil, nil, false, this.nodeManager.GetMachineID(), "")
	body := NewMessageBody(p2pMsgid, hash, 0, nil, 0) //广播采用同步机制后，不需要真实msgid，所以设置为0
	msg := NewMessage(head, body)
	msg.BuildHash()
	//如果是普通节点，则不需要对方回复
	if !reply {
		if ss, ok := this.sessionEngine.GetSessionAll(this.areaName, utils.Bytes2string(sid)); ok {
			// ss.Send(version_multicast, msg.Head.JSON(), msg.Body.JSON(), false)
			mheadBs := msg.Head.Proto()
			mbodyBs, err := msg.Body.Proto()
			if err != nil {
				utils.Log.Warn().Msgf("message body proto error:%s", err.Error())
			} else {
				// 依次遍历所有session，进行发送消息
				for i := range ss {
					ss[i].Send(version_multicast, &mheadBs, &mbodyBs, timeout)
				}
			}
		} else {
			// utils.Log.Warn().Msgf("get node conn fail %s", sid.B58String())
		}
		<-cs
		group.Done()
		return true
	}
	// utils.Log.Info().Msgf("multicast to:%s %s", sid.B58String(), hex.EncodeToString(msg.Body.Hash))

	if ss, ok := this.sessionEngine.GetSession(this.areaName, utils.Bytes2string(sid)); ok {
		// err := ss.Send(version_multicast, msg.Head.JSON(), msg.Body.JSON(), false)
		mheadBs := msg.Head.Proto()
		mbodyBs, err := msg.Body.Proto()
		flood.RegisterRequest(utils.Bytes2string(msg.Body.Hash))
		defer flood.RemoveRequest(utils.Bytes2string(msg.Body.Hash))
		err = ss.Send(msgid, &mheadBs, &mbodyBs, timeout)
		if err != nil {
			// utils.Log.Warn().Msgf("send multicast message fail %s", err.Error()) // fmt.Println("发送广播消息失败", err)
			// continue
		} else {
			// hashStr := hex.EncodeToString(message.Body.Hash)
			// utils.Log.Warn().Msgf("广播消息 1111111111111 %s", hashStr)
			// bs := flood.WaitRequest(config.CLASS_wallet_broadcast_return, hex.EncodeToString(message.Body.Hash), int64(i+1))
			// _, err := flood.WaitRequest(config.CLASS_wallet_broadcast_return, utils.Bytes2string(msg.Body.Hash), int64(4))
			_, err := flood.WaitResponse(utils.Bytes2string(msg.Body.Hash), time.Second*4)
			if err != nil {
				// utils.Log.Warn().Msgf("Timeout receiving broadcast reply message %s %s", sid.B58String(), hex.EncodeToString(msg.Body.Hash))
				// failNode = append(failNode, broadcasts[j])
				// continue
			} else {
				success = true
			}
			// utils.Log.Warn().Msgf("收到广播回复消息  %s %s", addrStr, hashStr)
		}
	} else {
		// utils.Log.Warn().Msgf("get node conn fail :%s", sid.B58String())
	}
	<-cs
	group.Done()
	return
}

// /*
// 	注册广播消息
// */
// func Register_multicast_OneLevel(msgid uint64, handler MsgHandler) {
// 	router.Register(msgid, handler)
// }

// /*
// 	发送一个新的广播消息
// */
// func SendMulticastMsg_OneLevel(msgid uint64, content *[]byte) bool {
// 	// utils.Log.Info().Msgf("发送广播 start")
// 	//TODO 这里查询缓存中的节点信息，拿到节点的超级节点信息
// 	head := NewMessageHead(nil, nil, false)
// 	body := NewMessageBody(msgid, content, 0, nil, 0)
// 	message := NewMessage(head, body)
// 	message.BuildHash()

// 	// headBs := message.Head.JSON()
// 	// bodyBs := message.Body.JSON()

// 	//先保存这个消息到缓存
// 	err := new(sqlite3_db.MessageCache).Add(message.Body.Hash, head.Proto(), body.Proto())
// 	if err != nil {
// 		utils.Log.Error().Msgf(err.Error())
// 		return false
// 	}

// 	//先发送给超级节点
// 	// superNodes := nodeStore.GetLogicNodes()
// 	// superNodes = append(superNodes, nodeStore.GetNodesClient()...)

// 	// //再发送给内网节点
// 	// proxyNodes := nodeStore.GetProxyAll()

// 	// broadcastsAll(superNodes, proxyNodes, message)
// 	//先发送给超级节点
// 	whiltlistNodes := nodeStore.GetWhiltListNodes()

// 	superNodes := nodeStore.GetLogicNodes()
// 	superNodes = append(superNodes, nodeStore.GetNodesClient()...)

// 	//再发送给内网节点
// 	proxyNodes := nodeStore.GetProxyAll()

// 	BroadcastsAll(version_multicast, whiltlistNodes, superNodes, proxyNodes, &message.Body.Hash)
// 	// utils.Log.Info().Msgf("发送广播 end")
// 	return true

// }

// func broadcastsAll_OneLevel(whiltlistNodes, superNodes, proxyNodes []nodeStore.AddressNet, hash *[]byte) {

// 	start := time.Now()
// 	//给已发送的节点放map里，避免重复发送
// 	allNodes := make(map[string]bool)

// 	//先发送给超级节点
// 	// superNodes := nodeStore.GetLogicNodes()
// 	//排除重复的地址
// 	// superNodes = nodeStore.RemoveDuplicateAddress(superNodes)
// 	cs := make(chan bool, config.CPUNUM)
// 	group := new(sync.WaitGroup)
// 	for i, _ := range whiltlistNodes {
// 		sessionid := whiltlistNodes[i]
// 		//不要发送给自己
// 		if bytes.Equal(nodeStore.NodeSelf.IdInfo.Id, sessionid) {
// 			continue
// 		}
// 		_, ok := allNodes[utils.Bytes2string(sessionid)]
// 		if ok {
// 			utils.Log.Info().Msgf("repeat node addr: %s", sessionid.B58String())
// 			continue
// 		}
// 		allNodes[utils.Bytes2string(sessionid)] = false
// 		cs <- false
// 		group.Add(1)
// 		//区块广播给节点
// 		// utils.Log.Info().Msgf("multcast super node:%s", sessionid.B58String())
// 		go broadcastsOne(cs, sessionid, hash, group, true)
// 	}
// 	group.Wait()
// 	utils.Log.Info().Msgf("multicast whilt list node time %s", time.Now().Sub(start))

// 	start = time.Now()
// 	for i, _ := range superNodes {
// 		sessionid := superNodes[i]
// 		//不要发送给自己
// 		if bytes.Equal(nodeStore.NodeSelf.IdInfo.Id, sessionid) {
// 			continue
// 		}
// 		_, ok := allNodes[utils.Bytes2string(sessionid)]
// 		if ok {
// 			utils.Log.Info().Msgf("repeat node addr: %s", sessionid.B58String())
// 			continue
// 		}
// 		allNodes[utils.Bytes2string(sessionid)] = false
// 		cs <- false
// 		group.Add(1)
// 		//区块广播给节点
// 		// utils.Log.Info().Msgf("multcast super node:%s", sessionid.B58String())
// 		go broadcastsOne(cs, sessionid, hash, group, false)
// 	}
// 	group.Wait()
// 	utils.Log.Info().Msgf("multicast super node time %s", time.Now().Sub(start))
// 	start = time.Now()

// 	//再发送给内网节点
// 	// proxyNodes := nodeStore.GetProxyAll()
// 	//排除重复的地址
// 	// proxyNodes = nodeStore.RemoveDuplicateAddress(proxyNodes)
// 	for i, _ := range proxyNodes {
// 		sessionid := proxyNodes[i]
// 		//不要发送给自己
// 		if bytes.Equal(nodeStore.NodeSelf.IdInfo.Id, sessionid) {
// 			continue
// 		}
// 		_, ok := allNodes[utils.Bytes2string(sessionid)]
// 		if ok {
// 			utils.Log.Info().Msgf("repeat node addr: %s", sessionid.B58String())
// 			continue
// 		}
// 		allNodes[utils.Bytes2string(sessionid)] = false
// 		cs <- false
// 		group.Add(1)
// 		//区块广播给节点
// 		go broadcastsOne(cs, sessionid, hash, group, false)
// 	}
// 	group.Wait()
// 	utils.Log.Info().Msgf("multicast proxy node time %s", time.Now().Sub(start))
// }

// /*
// 	给一个节点发送广播消息
// 	@return    bool    白名单种节点是否都收到了消息
// */
// func broadcastsOne_OneLevel(cs chan bool, sid nodeStore.AddressNet, hash *[]byte, group *sync.WaitGroup, reply bool) (success bool) {
// 	// utils.Log.Info().Msgf("multicast to %s", sid.B58String())
// 	//先查询对方的版本,看协议是否有升级
// 	// node := nodeStore.FindNode(&sid)
// 	// if node == nil {
// 	// 	<-cs
// 	// 	group.Done()
// 	// 	return
// 	// }
// 	// if nodeStore.NodeSelf.Version >= config.Version_1 && node.Version >= config.Version_1 {

// 	// }
// 	head := NewMessageHead(nil, nil, false)
// 	body := NewMessageBody(0, hash, 0, nil, 0) //广播采用同步机制后，不需要真实msgid，所以设置为0
// 	msg := NewMessage(head, body)
// 	msg.BuildHash()
// 	//如果是普通节点，则不需要对方回复
// 	if !reply {
// 		if ss, ok := engine.GetSession(utils.Bytes2string(sid)); ok {
// 			// ss.Send(version_multicast, msg.Head.JSON(), msg.Body.JSON(), false)
// 			mheadBs := msg.Head.Proto()
// 			mbodyBs := msg.Body.Proto()
// 			ss.Send(version_multicast, &mheadBs, &mbodyBs, false)
// 		} else {
// 			utils.Log.Warn().Msgf("get node conn fail %s", sid.B58String())
// 		}
// 		<-cs
// 		group.Done()
// 		return true
// 	}
// 	utils.Log.Info().Msgf("multicast to:%s %s", sid.B58String(), hex.EncodeToString(msg.Body.Hash))

// 	if ss, ok := engine.GetSession(utils.Bytes2string(sid)); ok {
// 		// err := ss.Send(version_multicast, msg.Head.JSON(), msg.Body.JSON(), false)
// 		mheadBs := msg.Head.Proto()
// 		mbodyBs := msg.Body.Proto()
// 		err := ss.Send(version_multicast, &mheadBs, &mbodyBs, false)
// 		if err != nil {
// 			utils.Log.Warn().Msgf("send multicast message fail %s", err.Error()) // fmt.Println("发送广播消息失败", err)
// 			// continue
// 		} else {
// 			// hashStr := hex.EncodeToString(message.Body.Hash)
// 			// utils.Log.Warn().Msgf("广播消息 1111111111111 %s", hashStr)
// 			// bs := flood.WaitRequest(config.CLASS_wallet_broadcast_return, hex.EncodeToString(message.Body.Hash), int64(i+1))
// 			_, err := flood.WaitRequest(config.CLASS_wallet_broadcast_return, utils.Bytes2string(msg.Body.Hash), int64(4))
// 			if err != nil {
// 				utils.Log.Warn().Msgf("Timeout receiving broadcast reply message %s %s", sid.B58String(), hex.EncodeToString(msg.Body.Hash))
// 				// failNode = append(failNode, broadcasts[j])
// 				// continue
// 			} else {
// 				success = true
// 			}
// 			// utils.Log.Warn().Msgf("收到广播回复消息  %s %s", addrStr, hashStr)
// 		}
// 	} else {
// 		utils.Log.Warn().Msgf("get node conn fail :%s", sid.B58String())
// 	}
// 	<-cs
// 	group.Done()
// 	return
// }

/*
注册消息
从超级节点中搜索目标节点
*/
func (this *MessageCenter) Register_search_super(msgid uint64, handler MsgHandler, isSysMsg bool) {
	this.router.Register(msgid, handler, isSysMsg)
}

/*
发送一个新的查找超级节点消息
@return    bool    是否发送给其他节点:true=发送成功;false=发送失败;
*/
func (this *MessageCenter) SendSearchSuperMsg(msgid uint64, recvid *nodeStore.AddressNet, content *[]byte) (*Message, error) {
	if !this.CheckOnline() {
		return nil, config.ERROR_offline
	}
	if recvid == nil || len(*recvid) == 0 {
		return nil, config.ERROR_params_fail
	}
	//搜索接口不能吧消息发送给自己
	if bytes.Equal(this.nodeManager.NodeSelf.IdInfo.Id, *recvid) {
		return nil, config.ERROR_sent_myself
	}

	mhead := NewMessageHead(this.nodeManager.NodeSelf, this.nodeManager.GetSuperPeerId(), recvid, recvid, false, this.nodeManager.GetMachineID(), "")
	mbody := NewMessageBody(msgid, content, 0, nil, 0)
	message := NewMessage(mhead, mbody)
	message.BuildHash()
	isOther, err := message.Send(version_search_super, this.nodeManager, this.sessionEngine, this.vm, 0)
	if err != nil {
		return nil, err
	}
	if !isOther {
		this.magneticSelf(message)
	}
	return message, nil
}

/*
发送一个新的查找超级节点消息
*/
func (this *MessageCenter) SendSearchSuperMsgWaitRequest(msgid uint64, recvid *nodeStore.AddressNet,
	content *[]byte, timeout time.Duration, oneByOne bool) (*[]byte, error) {
	if !this.CheckOnline() {
		return nil, config.ERROR_offline
	}
	if recvid == nil || len(*recvid) == 0 {
		return nil, config.ERROR_params_fail
	}
	mhead := NewMessageHeadOneByOne(this.nodeManager.NodeSelf, this.nodeManager.GetSuperPeerId(), recvid, recvid, false, oneByOne, this.nodeManager.GetMachineID(), "")
	mbody := NewMessageBody(msgid, content, 0, nil, 0)
	message := NewMessage(mhead, mbody)

	message.BuildHash()

	flood.RegisterRequest(utils.Bytes2string(message.Body.Hash))
	defer flood.RemoveRequest(utils.Bytes2string(message.Body.Hash))
	isOther, err := message.Send(version_search_super, this.nodeManager, this.sessionEngine, this.vm, timeout)
	if err != nil {
		return nil, err
	}
	if !isOther {
		this.magneticSelf(message)
	}

	return flood.WaitResponse(utils.Bytes2string(message.Body.Hash), timeout)
}

/*
对某个消息回复
*/
func (this *MessageCenter) SendSearchSuperReplyMsg(message *Message, msgid uint64, content *[]byte) error {
	return this.SendP2pReplyMsg(message, msgid, content)
}

/*
注册消息
从所有节点中搜索节点，包括普通节点
*/
func (this *MessageCenter) Register_search_all(msgid uint64, handler MsgHandler, isSysMsg bool) {
	this.router.Register(msgid, handler, isSysMsg)
}

/*
发送一个新的查找超级节点消息
*/
func (this *MessageCenter) SendSearchAllMsg(msgid uint64, recvid *nodeStore.AddressNet, content *[]byte) (*Message, error) {
	if !this.CheckOnline() {
		return nil, config.ERROR_offline
	}
	if recvid == nil || len(*recvid) == 0 {
		return nil, config.ERROR_params_fail
	}
	mhead := NewMessageHead(this.nodeManager.NodeSelf, this.nodeManager.GetSuperPeerId(), recvid, recvid, false, this.nodeManager.GetMachineID(), "")
	mbody := NewMessageBody(msgid, content, 0, nil, 0)
	message := NewMessage(mhead, mbody)
	message.BuildHash()

	isOther, err := message.Send(version_search_all, this.nodeManager, this.sessionEngine, this.vm, 0)
	if err != nil {
		return nil, err
	}
	if !isOther {
		this.magneticSelf(message)
	}
	return message, nil
}

/*
对某个消息回复
*/
func (this *MessageCenter) SendSearchAllReplyMsg(message *Message, msgid uint64, content *[]byte) error {
	return this.SendP2pReplyMsg(message, msgid, content)
}

/*
注册点对点通信消息
*/
func (this *MessageCenter) Register_p2p(msgid uint64, handler MsgHandler, isSysMsg bool) {
	this.router.Register(msgid, handler, isSysMsg)
}

/*
给指定节点发送一个消息
@return    *Message     返回的消息
@return    bool         是否发送成功
@return    bool         消息是发给自己
@return    error        发送失败
*/
func (this *MessageCenter) SendP2pMsg(msgid uint64, recvid *nodeStore.AddressNet, content *[]byte) (*Message, bool, bool, error) {
	if !this.CheckOnline() {
		return nil, false, false, config.ERROR_offline
	}
	if recvid == nil || len(*recvid) == 0 {
		return nil, false, false, config.ERROR_params_fail
	}

	//TODO 这里查询缓存中的节点信息，拿到节点的超级节点信息
	// utils.Log.Info().Msgf("发送未加密消息 1111111111111 msgid:%d", msgid)
	head := NewMessageHead(this.nodeManager.NodeSelf, this.nodeManager.GetSuperPeerId(), recvid, recvid, true, this.nodeManager.GetMachineID(), "")
	body := NewMessageBody(msgid, content, 0, nil, 0)
	message := NewMessage(head, body)
	message.BuildHash()

	if bytes.Equal(this.nodeManager.NodeSelf.IdInfo.Id, *recvid) {
		this.magneticSelf(message)
		return message, true, true, nil
	}

	isOther, err := message.Send(version_p2p, this.nodeManager, this.sessionEngine, this.vm, 0)
	if err != nil {
		return nil, false, !isOther, err
	}
	if !isOther {
		this.magneticSelf(message)
	}
	// utils.Log.Info().Msgf("发送未加密消息 2222222222222 msgid:%d ok:%t", msgid, ok)
	return message, true, !isOther, nil
}

/*
给指定节点发送一个消息
@return    *[]byte      返回的内容
@return    bool         是否发送成功
@return    bool         消息是发给自己
*/
func (this *MessageCenter) SendP2pMsgWaitRequest(msgid uint64, recvid *nodeStore.AddressNet,
	content *[]byte, timeout time.Duration) (*[]byte, bool, bool, error) {
	if !this.CheckOnline() {
		return nil, false, false, config.ERROR_offline
	}
	if recvid == nil || len(*recvid) == 0 {
		return nil, false, false, config.ERROR_params_fail
	}
	//TODO 这里查询缓存中的节点信息，拿到节点的超级节点信息
	// utils.Log.Info().Msgf("发送未加密消息 1111111111111 msgid:%d", msgid)
	head := NewMessageHead(this.nodeManager.NodeSelf, this.nodeManager.GetSuperPeerId(), recvid, recvid, true, this.nodeManager.GetMachineID(), "")
	body := NewMessageBody(msgid, content, 0, nil, 0)
	message := NewMessage(head, body)
	message.BuildHash()
	flood.RegisterRequest(utils.Bytes2string(message.Body.Hash))
	defer flood.RemoveRequest(utils.Bytes2string(message.Body.Hash))
	if bytes.Equal(this.nodeManager.NodeSelf.IdInfo.Id, *recvid) {
		this.magneticSelf(message)
		bs, err := flood.WaitResponse(utils.Bytes2string(message.Body.Hash), timeout)
		if err != nil {
			return nil, true, true, err
		}
		return bs, true, true, nil
	}
	isOther, err := message.Send(version_p2p, this.nodeManager, this.sessionEngine, this.vm, timeout)
	if err != nil {
		return nil, false, !isOther, err
	}
	if !isOther {
		this.magneticSelf(message)
	}
	bs, err := flood.WaitResponse(utils.Bytes2string(message.Body.Hash), timeout)
	if err != nil {
		return nil, true, !isOther, err
	}
	return bs, true, !isOther, nil
}

/*
给指定节点发送一个消息
是SendP2pMsg方法的定制版本，多了recvSuperId参数。
*/
func (this *MessageCenter) SendP2pMsgEX(msgid uint64, recvid, recvSuperId *nodeStore.AddressNet, content *[]byte, hash *[]byte) (*Message, error) {
	if !this.CheckOnline() {
		return nil, config.ERROR_offline
	}
	if recvid == nil || len(*recvid) == 0 {
		return nil, config.ERROR_params_fail
	}
	if recvSuperId == nil || len(*recvSuperId) == 0 {
		return nil, config.ERROR_params_fail
	}

	//TODO 这里查询缓存中的节点信息，拿到节点的超级节点信息
	head := NewMessageHead(this.nodeManager.NodeSelf, this.nodeManager.GetSuperPeerId(), recvid, recvSuperId, true, this.nodeManager.GetMachineID(), "")
	body := NewMessageBody(msgid, content, 0, *hash, 0)
	message := NewMessage(head, body)
	message.BuildHash()
	isOther, err := message.Send(version_p2p, this.nodeManager, this.sessionEngine, this.vm, 0)
	if err != nil {
		return nil, err
	}
	if !isOther {
		this.magneticSelf(message)
	}
	return message, nil
}

/*
给指定节点发送一个消息
是SendP2pMsg方法的定制版本，多了recvSuperId、recvProxId和senderProxyId参数。
*/
func (this *MessageCenter) SendP2pMsgEXWaitRequest(msgid uint64, recvid, recvSuperId, recvProxyId, senderProxyId *nodeStore.AddressNet, recvMachineId string,
	content *[]byte, timeout time.Duration) (*[]byte, error) {
	if !this.CheckOnline() {
		return nil, config.ERROR_offline
	}
	if recvid == nil || len(*recvid) == 0 {
		return nil, config.ERROR_params_fail
	}
	if recvSuperId == nil || len(*recvSuperId) == 0 {
		return nil, config.ERROR_params_fail
	}
	//TODO 这里查询缓存中的节点信息，拿到节点的超级节点信息
	head := NewMessageHeadProxy(this.nodeManager.NodeSelf, this.nodeManager.GetSuperPeerId(), recvid, recvSuperId, recvProxyId, senderProxyId, true, this.nodeManager.GetMachineID(), recvMachineId)
	body := NewMessageBody(msgid, content, 0, nil, 0)
	message := NewMessage(head, body)
	message.BuildHash()
	flood.RegisterRequest(utils.Bytes2string(message.Body.Hash))
	defer flood.RemoveRequest(utils.Bytes2string(message.Body.Hash))
	isOther, err := message.Send(version_p2p, this.nodeManager, this.sessionEngine, this.vm, timeout)
	if err != nil {
		return nil, err
	}
	if !isOther {
		this.magneticSelf(message)
	}
	return flood.WaitResponse(utils.Bytes2string(message.Body.Hash), timeout)
	// return message, nil
}

/*
对某个消息回复
*/
func (this *MessageCenter) SendP2pReplyMsg(message *Message, msgid uint64, content *[]byte) error {
	if !this.CheckOnline() {
		return config.ERROR_offline
	}
	mhead := NewMessageHeadProxy(this.nodeManager.NodeSelf, this.nodeManager.GetSuperPeerId(), message.Head.Sender, message.Head.SenderSuperId, message.Head.SenderProxyId, nil, true, this.nodeManager.GetMachineID(), message.Head.SenderMachineID)
	mbody := NewMessageBody(msgid, content, message.Body.CreateTime, message.Body.Hash, message.Body.SendRand)
	newmessage := NewMessage(mhead, mbody)
	newmessage.BuildReplyHash(message.Body.CreateTime, message.Body.Hash, message.Body.SendRand)
	isOther, err := newmessage.Reply(version_p2p, this.nodeManager, this.sessionEngine, this.vm)
	if err != nil {
		return err
	}
	if !isOther {
		this.magneticSelf(newmessage)
	}
	return nil
}

/*
注册点对点通信消息
*/
func (this *MessageCenter) Register_p2pHE(msgid uint64, handler MsgHandler, isSysMsg bool) {
	this.router.Register(msgid, handler, isSysMsg)
}

// var securityStore = new(sync.Map)

/*
发送一个加密消息，包括消息头也加密
@return    *Message     返回的消息
@return    bool         是否发送成功
@return    bool         消息是发给自己
*/
func (this *MessageCenter) SendP2pMsgHE(msgid uint64, recvid *nodeStore.AddressNet, content *[]byte) (*Message, bool, bool, error) {
	if !this.CheckOnline() {
		return nil, false, false, config.ERROR_offline
	}
	if recvid == nil || len(*recvid) == 0 {
		return nil, false, false, config.ERROR_params_fail
	}
	if content == nil {
		bs := []byte{0}
		content = &bs
	}
	message, err := this.sendP2pMsgHELoadRatchet(msgid, recvid, nil, nil, "", content)
	if err != nil {
		return nil, false, false, err
	}
	if bytes.Equal(this.nodeManager.NodeSelf.IdInfo.Id, *recvid) {
		message.Body.Content = content
		// utils.Log.Info().Msgf("是发送给自己")
		// utils.Log.Info().Msgf("是发送给自己:%d %s", len(*message.Body.Content), hex.EncodeToString(*message.Body.Content))
		this.magneticSelf(message)
		return message, true, true, nil
	}
	isOther, err := message.Send(version_p2pHE, this.nodeManager, this.sessionEngine, this.vm, 0)
	if err != nil {
		return nil, false, !isOther, err
	}
	if !isOther {
		this.magneticSelf(message)
	}
	// utils.Log.Info().Msgf("============ 发送加密消息 33333333333")
	return message, true, !isOther, nil
}

/*
发送一个加密消息，包括消息头也加密
@return    *Message     返回的消息
@return    bool         是否发送成功
@return    bool         消息是发给自己
*/
func (this *MessageCenter) SendP2pMsgHEWaitRequest(msgid uint64, recvid *nodeStore.AddressNet,
	content *[]byte, timeout time.Duration) (*[]byte, bool, bool, error) {
	if !this.CheckOnline() {
		return nil, false, false, config.ERROR_offline
	}
	if recvid == nil || len(*recvid) == 0 {
		return nil, false, false, config.ERROR_params_fail
	}
	if content == nil {
		bs := []byte{0}
		content = &bs
	}
	message, err := this.sendP2pMsgHELoadRatchet(msgid, recvid, nil, nil, "", content)
	if err != nil {
		return nil, false, false, err
	}
	flood.RegisterRequest(utils.Bytes2string(message.Body.Hash))
	defer flood.RemoveRequest(utils.Bytes2string(message.Body.Hash))
	if bytes.Equal(this.nodeManager.NodeSelf.IdInfo.Id, *recvid) {
		message.Body.Content = content
		this.magneticSelf(message)
		bs, err := flood.WaitResponse(utils.Bytes2string(message.Body.Hash), timeout)
		return bs, true, true, err
		// return nil, false, true, nil
	}

	isOther, err := message.Send(version_p2pHE, this.nodeManager, this.sessionEngine, this.vm, timeout)
	if err != nil {
		return nil, false, !isOther, err
	}
	if !isOther {
		this.magneticSelf(message)
	}
	bs, err := flood.WaitResponse(utils.Bytes2string(message.Body.Hash), timeout)
	return bs, true, !isOther, err
	// utils.Log.Info().Msgf("============ 发送加密消息 33333333333")
	// return message, true, false, nil
}

/*
发送一个加密消息，包括消息头也加密
@return    *Message     返回的消息
@return    bool         是否发送成功
@return    bool         消息是发给自己
*/
func (this *MessageCenter) sendP2pMsgHELoadRatchet(msgid uint64, recvid, recvProxyId, senderProxyId *nodeStore.AddressNet, recvMachineId string, content *[]byte) (*Message, error) {
	unlock := createHeMutex.Lock(utils.Bytes2string(*recvid))
	defer unlock()

	//这里查询缓存中的节点信息，拿到节点的超级节点信息
	ratchet := this.RatchetSession.GetSendRatchet(*recvid, recvMachineId)
	strKey := utils.Bytes2string(*recvid)

	this.securityStoreLock.Lock()
	this.securityStoreLock.Unlock()

	exist := true
	machineValue, ok := this.securityStore[strKey]
	if ok && machineValue != nil && recvMachineId != "" {
		_, exist = machineValue[recvMachineId]
	}
	if ratchet == nil || !ok || !exist {
		//utils.Log.Info().Msgf("============ 没有协商密钥")
		// 判断是否含有代理节点信息
		bHaveProxyInfo := true
		if (recvProxyId == nil || len(*recvProxyId) == 0) && (senderProxyId == nil || len(*senderProxyId) == 0) {
			bHaveProxyInfo = false
		}

		//发送获取节点信息消息
		//获取对方身份公钥
		var bs *[]byte
		var err error
		if !bHaveProxyInfo {
			//utils.Log.Info().Msgf("============ 发送获取节点信息消息")
			// 没有接收和发送代理节点信息
			bs, _, _, err = this.SendP2pMsgWaitRequest(config.MSGID_SearchAddr, recvid, nil, time.Second*10)
		} else {
			//utils.Log.Info().Msgf("============ 发送获取节点信息消息proxy")
			bs, _, _, err = this.SendP2pMsgProxyWaitRequest(config.MSGID_SearchAddr, recvid, recvProxyId, senderProxyId, recvMachineId, nil, time.Second*10)
		}
		if err != nil {
			utils.Log.Info().Msgf("获取对方身份公钥失败:%s", err.Error())
			return nil, err
		}
		if bs == nil {
			// utils.Log.Info().Msgf("============ 发送加密消息 22222")
			return nil, nil
		}
		// fmt.Println("============ 发送加密消息 33333333")
		sni, err := ParserSearchNodeInfo(*bs)
		if err != nil {
			// utils.Log.Info().Msgf("============ 发送加密消息 4444444")
			return nil, nil
		}
		// utils.Log.Info().Msgf("============ 发送加密消息 55555555")

		//生成3个密钥，通过共享密钥加密发送两个密钥
		// ivRand, err := crypto.Rand32Byte()
		// if err != nil {
		// 	return nil, false
		// }
		// ivDH, err := dh.GenerateKeyPair(ivRand)
		// if err != nil {
		// 	return nil, false
		// }
		// iv := dh.KeyExchange(dh.DHPair{PrivateKey: ivDH.GetPrivateKey(), PublicKey: sni.CPuk})

		//获取一个随机数
		sharedHkaRand, err := crypto.Rand32Byte()
		if err != nil {
			return nil, err
		}
		// utils.Log.Info().Msgf("============ 发送加密消息 666666666")
		//使用随机数生成一个密钥对
		sharedHkaDH, err := dh.GenerateKeyPair(sharedHkaRand[:])
		if err != nil {
			return nil, err
		}
		// utils.Log.Info().Msgf("============ 发送加密消息 7777777777")
		//生成一个共享密钥Hka
		sharedHka, err := dh.KeyExchange(dh.NewDHPair(sharedHkaDH.GetPrivateKey(), sni.CPuk))
		if err != nil {
			return nil, err
		}
		//获取一个随机数
		sharedNhkbRand, err := crypto.Rand32Byte()
		if err != nil {
			return nil, err
		}
		// utils.Log.Info().Msgf("============ 发送加密消息 88888888")
		//使用随机数生成一个密钥对
		sharedNhkbDH, err := dh.GenerateKeyPair(sharedNhkbRand[:])
		if err != nil {
			return nil, err
		}
		// utils.Log.Info().Msgf("============ 发送加密消息 99999")
		//使用自己的私钥和对方公钥生成共享密钥Nhkb
		sharedNhkb, err := dh.KeyExchange(dh.NewDHPair(sharedNhkbDH.GetPrivateKey(), sni.CPuk))
		if err != nil {
			return nil, err
		}

		//生成共享密钥SK
		sk, err := dh.KeyExchange(dh.NewDHPair(this.key.GetDHKeyPair().KeyPair.GetPrivateKey(), sni.CPuk))
		if err != nil {
			return nil, err
		}
		// cpuk := keystore.GetDHKeyPair().KeyPair.GetPublicKey()
		// cpukNet := nodeStore.NodeSelf.IdInfo.CPuk
		shareKey := ShareKey{
			Idinfo:   this.nodeManager.NodeSelf.IdInfo,
			A_DH_PUK: sharedHkaDH.GetPublicKey(),  //
			B_DH_PUK: sharedNhkbDH.GetPublicKey(), //B公钥
		}
		*bs, err = shareKey.Proto() //
		if err != nil {
			return nil, err
		}
		//utils.Log.Info().Msgf("============ 给对方发送自己的公钥，用于创建加密通道消息")
		//给对方发送自己的公钥，用于创建加密通道消息
		bs, err = this.SendP2pMsgEXWaitRequest(config.MSGID_security_create_pipe, sni.Id, sni.SuperId, recvProxyId, senderProxyId, recvMachineId, bs, time.Second*10)
		if err != nil {
			// utils.Log.Info().Msgf("============ 发送加密消息 1010101022222")
			return nil, err
		}
		if bs == nil {
			// utils.Log.Info().Msgf("============ 发送加密消息 1010101022222")
			return nil, errors.New("bs is nil")
		}
		// utils.Log.Info().Msgf("============ 发送加密消息 111111112222222")
		err = this.RatchetSession.AddSendPipe(*sni.Id, recvMachineId, sk, sharedHka, sharedNhkb, this.key)
		if err != nil {
			// utils.Log.Info().Msgf("============ 发送加密消息 1010101022222")
			return nil, err
		}
		// utils.Log.Info().Msgf("============ 发送加密消息 22222222221111111")
		//
		machineValue = make(map[string]*SearchNodeInfo)
		machineValue[recvMachineId] = sni
		this.securityStore[strKey] = machineValue
	}
	//utils.Log.Info().Msgf("============ 发送加密消息 22222")
	machineValue, ok = this.securityStore[strKey]
	if !ok {
		return nil, errors.New("securityStore not exist")
	}

	var sni *SearchNodeInfo
	if recvMachineId != "" {
		sni, ok = machineValue[recvMachineId]
		if !ok {
			return nil, errors.New("searchNodeInfo not exist")
		}
	} else {
		for _, sni = range machineValue {
			if sni != nil {
				break
			}
		}
	}
	if sni == nil {
		return nil, errors.New("searchNodeInfo not exist")
	}

	//utils.Log.Info().Msgf("============ 发送加密消息 444444")
	//开始发送真正的消息
	//先将内容加密
	ratchet = this.RatchetSession.GetSendRatchet(*sni.Id, recvMachineId)
	if ratchet == nil {
		// utils.Log.Info().Msgf("============ 发送加密消息 5555555")
		return nil, errors.New("sendRatchet not exist")
	}
	//utils.Log.Info().Msgf("============ 发送加密消息 6666666")

	msgHE := ratchet.RatchetEncrypt(*content, nil)
	buf := bytes.NewBuffer(nil)
	binary.Write(buf, binary.LittleEndian, uint64(len(msgHE.Header)))
	buf.Write(msgHE.Header)
	binary.Write(buf, binary.LittleEndian, uint64(len(msgHE.Ciphertext)))
	buf.Write(msgHE.Ciphertext)
	bs := buf.Bytes()
	// fmt.Println("head", hex.EncodeToString(msgHE.Header))
	// fmt.Println("body", hex.EncodeToString(msgHE.Ciphertext))

	head := NewMessageHeadProxy(this.nodeManager.NodeSelf, this.nodeManager.GetSuperPeerId(), sni.Id, sni.SuperId, recvProxyId, senderProxyId, true, this.nodeManager.GetMachineID(), recvMachineId)
	body := NewMessageBody(msgid, &bs, 0, nil, 0)
	message := NewMessage(head, body)
	message.BuildHash()
	return message, nil
}

/*
对某个消息回复
*/
func (this *MessageCenter) SendP2pReplyMsgHE(message *Message, msgid uint64, content *[]byte) error {
	if !this.CheckOnline() {
		return config.ERROR_offline
	}
	mhead := NewMessageHeadProxy(this.nodeManager.NodeSelf, this.nodeManager.GetSuperPeerId(), message.Head.Sender, message.Head.SenderSuperId, message.Head.SenderProxyId, nil, true, this.nodeManager.GetMachineID(), message.Head.SenderMachineID)
	mbody := NewMessageBody(msgid, content, message.Body.CreateTime, message.Body.Hash, message.Body.SendRand)
	newmessage := NewMessage(mhead, mbody)
	// newmessage.BuildReplyHash(message.Body.CreateTime, message.Body.Hash, message.Body.SendRand)
	// if !newmessage.Reply(version_p2p, this.nodeManager, this.sessionEngine) {
	// 	this.magneticSelf(newmessage)
	// }

	isOther, err := newmessage.Reply(version_p2p, this.nodeManager, this.sessionEngine, this.vm)
	if err != nil {
		return err
	}
	if !isOther {
		this.magneticSelf(newmessage)
	}
	return nil
}

/*
注册虚拟节点搜索节点消息
*/
func (this *MessageCenter) Register_vnode_search(msgid uint64, handler MsgHandler, isSysMsg bool) {
	this.router.Register(msgid, handler, isSysMsg)
}

/*
发送虚拟节点搜索节点消息
*/
func (this *MessageCenter) SendVnodeSearchMsg(msgid uint64, sendVnodeid, recvVnodeid *virtual_node.AddressNetExtend, recvProxyId, senderProxyId *nodeStore.AddressNet, content *[]byte) (*Message, error) {
	// fmt.Println("------------------\n发送一个虚拟节点搜索节点消息")
	if !this.CheckOnline() {
		return nil, config.ERROR_offline
	}

	if sendVnodeid == nil || len(*sendVnodeid) == 0 {
		return nil, config.ERROR_params_fail
	}
	if recvVnodeid == nil || len(*recvVnodeid) == 0 {
		return nil, config.ERROR_params_fail
	}
	//直接查找最近的超级节点
	mhead := NewMessageHeadVnode(&this.nodeManager.NodeSelf.IdInfo.Id, sendVnodeid, recvVnodeid, recvProxyId, senderProxyId, false, this.nodeManager.GetMachineID(), "")
	mbody := NewMessageBody(msgid, content, 0, nil, 0)
	message := NewMessage(mhead, mbody)
	message.BuildHash()
	isOther, err := message.Send(version_vnode_search, this.nodeManager, this.sessionEngine, this.vm, 0)
	if err != nil {
		return nil, err
	}
	if !isOther {
		this.magneticSelf(message)
	}
	return message, nil
}

/*
发送到目标节点时用 虚拟节点onebyone规则搜索节点
目标节点发回时使用p2p消息
发送 虚拟节点 搜索节点消息
*/
func (this *MessageCenter) SendVnodeSearchMsgWaitRequest(msgid uint64, sendVnodeid, recvVnodeid *virtual_node.AddressNetExtend, recvProxyId, senderProxyId *nodeStore.AddressNet, content *[]byte, timeout time.Duration, onebyone bool) (*[]byte, error) {
	// utils.Log.Info().Msgf("SendVnodeSearchMsgWaitRequest start")
	// defer utils.Log.Info().Msgf("SendVnodeSearchMsgWaitRequest end")
	if !this.CheckOnline() {
		return nil, config.ERROR_offline
	}
	if sendVnodeid == nil || len(*sendVnodeid) == 0 {
		return nil, config.ERROR_params_fail
	}
	if recvVnodeid == nil || len(*recvVnodeid) == 0 {
		return nil, config.ERROR_params_fail
	}

	//utils.Log.Info().Msgf("SendVnodeSearchMsgWaitRequest : %t", onebyone)
	//TODO 这里查询缓存中的节点信息，拿到节点的超级节点信息
	head := NewMessageHeadVnode(&this.nodeManager.NodeSelf.IdInfo.Id, sendVnodeid, recvVnodeid, recvProxyId, senderProxyId, false, this.nodeManager.GetMachineID(), "")
	head.OneByOne = onebyone
	head.SenderSuperId = this.nodeManager.GetSuperPeerId()
	body := NewMessageBody(msgid, content, 0, nil, 0)
	message := NewMessage(head, body)
	message.BuildHash()
	flood.RegisterRequest(utils.Bytes2string(message.Body.Hash))
	defer flood.RemoveRequest(utils.Bytes2string(message.Body.Hash))

	if timeout < time.Second {
		timeout = time.Second
	}

	isOther, err := message.Send(version_vnode_search, this.nodeManager, this.sessionEngine, this.vm, timeout)
	if err != nil {
		utils.Log.Info().Msgf("SendVnodeSearchMsgWaitRequest 发送就失败了:%s", err.Error())
		return nil, err
	}
	if !isOther {
		// utils.Log.Info().Msgf("执行自己节点的方法 start")
		this.magneticSelf(message)
		// utils.Log.Info().Msgf("执行自己节点的方法 end")
	}
	return flood.WaitResponse(utils.Bytes2string(message.Body.Hash), timeout)
}

/*
对发送虚拟节点搜索节点消息回复
*/
func (this *MessageCenter) SendVnodeSearchReplyMsg(message *Message, msgid uint64, content *[]byte, onebyone bool) error {
	return this.SendVnodeP2pReplyMsgHE(message, msgid, content, onebyone)
	// if !this.CheckOnline() {
	// 	return config.ERROR_offline
	// }
	// // mhead := NewMessageHeadVnode(&this.nodeManager.NodeSelf.IdInfo.Id, sendVnodeid, recvVnodeid, false)

	// mhead := NewMessageHead(this.nodeManager.NodeSelf, this.nodeManager.SuperPeerId, message.Head.Sender, message.Head.Sender, true)
	// mhead.SenderVnode = message.Head.SelfVnodeId
	// mhead.RecvVnode = message.Head.SenderVnode
	// mbody := NewMessageBody(msgid, content, message.Body.CreateTime, message.Body.Hash, message.Body.SendRand)
	// newmessage := NewMessage(mhead, mbody)
	// // newmessage.BuildReplyHash(message.Body.CreateTime, message.Body.Hash, message.Body.SendRand)
	// // if !newmessage.Reply(version_p2p, this.nodeManager, this.sessionEngine) {
	// // 	this.magneticSelf(newmessage)
	// // }

	// bs, _ := mhead.JSON()
	// utils.Log.Info().Msgf("SendVnodeSearchReplyMsg: %s", string(bs))

	// if this.vm.FindInVnodeSelf(*mhead.RecvVnode) {
	// 	newmessage.Body.Content = content
	// 	this.magneticSelf(newmessage)
	// 	return nil
	// }

	// isOther, err := newmessage.Reply(version_vnode_p2pHE, this.nodeManager, this.sessionEngine, this.vm)
	// if err != nil {
	// 	return err
	// }
	// if !isOther {
	// 	this.magneticSelf(newmessage)
	// }
	// return nil

	// // return this.SendVnodeP2pReplyMsgHE(message, msgid, content, session)
}

/*
注册虚拟节点之间点对点加密消息
*/
func (this *MessageCenter) Register_vnode_p2pHE(msgid uint64, handler MsgHandler, isSysMsg bool) {
	this.router.Register(msgid, handler, isSysMsg)
}

/*
发送虚拟节点之间点对点消息
*/
func (this *MessageCenter) SendVnodeP2pMsgHE(msgid uint64, sendVnodeid, recvVnodeid *virtual_node.AddressNetExtend, recvNodeId, recvProxyId, senderProxyId *nodeStore.AddressNet, recvMachineId string, content *[]byte) (*Message, error) {
	// utils.Log.Info().Msgf("------------------\n发送一个虚拟节点点对点加密消息")
	if !this.CheckOnline() {
		return nil, config.ERROR_offline
	}
	if sendVnodeid == nil || len(*sendVnodeid) == 0 {
		return nil, config.ERROR_params_fail
	}
	if recvVnodeid == nil || len(*recvVnodeid) == 0 {
		return nil, config.ERROR_params_fail
	}
	//直接查找最近的超级节点
	mhead := NewMessageHeadVnode(&this.nodeManager.NodeSelf.IdInfo.Id, sendVnodeid, recvVnodeid, recvProxyId, senderProxyId, true, this.nodeManager.GetMachineID(), recvMachineId)
	mhead.SenderSuperId = this.nodeManager.GetSuperPeerId()
	mhead.Sender = &this.nodeManager.NodeSelf.IdInfo.Id
	if recvNodeId != nil {
		mhead.RecvId = recvNodeId
	}
	mbody := NewMessageBody(msgid, content, 0, nil, 0)
	message := NewMessage(mhead, mbody)
	message.BuildHash()
	isOther, err := message.Send(version_vnode_p2pHE, this.nodeManager, this.sessionEngine, this.vm, 0)
	if err != nil {
		return nil, err
	}
	if !isOther {
		// utils.Log.Info().Msgf("执行自己节点的方法")
		this.magneticSelf(message)
	}
	return message, nil
}

/*
发送虚拟节点搜索节点消息
*/
func (this *MessageCenter) SendVnodeP2pMsgHEWaitRequest(msgid uint64, sendVnodeid, recvVnodeid *virtual_node.AddressNetExtend, recvNodeId, recvProxyId, senderProxyId *nodeStore.AddressNet, recvMachineId string, content *[]byte, timeout time.Duration) (*[]byte, error) {
	// utils.Log.Info().Msgf("------------------\n发送一个虚拟节点点对点加密消息")
	if !this.CheckOnline() {
		return nil, config.ERROR_offline
	}
	if sendVnodeid == nil || len(*sendVnodeid) == 0 || recvVnodeid == nil || len(*recvVnodeid) == 0 {
		return nil, config.ERROR_params_fail
	}
	//直接查找最近的超级节点
	mhead := NewMessageHeadVnode(&this.nodeManager.NodeSelf.IdInfo.Id, sendVnodeid, recvVnodeid, recvProxyId, senderProxyId, true, this.nodeManager.GetMachineID(), recvMachineId)
	mhead.SenderSuperId = this.nodeManager.GetSuperPeerId()
	mhead.Sender = &this.nodeManager.NodeSelf.IdInfo.Id
	if recvNodeId != nil {
		mhead.RecvId = recvNodeId
	}
	mbody := NewMessageBody(msgid, content, 0, nil, 0)
	message := NewMessage(mhead, mbody)
	message.BuildHash()
	flood.RegisterRequest(utils.Bytes2string(message.Body.Hash))
	defer flood.RemoveRequest(utils.Bytes2string(message.Body.Hash))
	if timeout < time.Second {
		timeout = time.Second
	}

	isOther, err := message.Send(version_vnode_p2pHE, this.nodeManager, this.sessionEngine, this.vm, timeout)
	if err != nil {
		utils.Log.Info().Msgf("错误:%s", err.Error())
		return nil, err
	}
	if !isOther {
		// utils.Log.Info().Msgf("执行自己节点的方法")
		this.magneticSelf(message)
	}
	return flood.WaitResponse(utils.Bytes2string(message.Body.Hash), timeout)
}

/*
对发送虚拟节点之间点对点消息回复
*/
func (this *MessageCenter) SendVnodeP2pReplyMsgHE(message *Message, msgid uint64, content *[]byte, onebyone bool) error {
	if !this.CheckOnline() {
		return config.ERROR_offline
	}

	head := NewMessageHeadVnode(&this.nodeManager.NodeSelf.IdInfo.Id, message.Head.SelfVnodeId, message.Head.SenderVnode, message.Head.SenderProxyId, nil, true, this.nodeManager.GetMachineID(), message.Head.SenderMachineID)
	head.RecvId = message.Head.Sender
	head.RecvSuperId = message.Head.SenderSuperId
	head.SenderSuperId = this.nodeManager.GetSuperPeerId()
	head.OneByOne = onebyone
	body := NewMessageBody(msgid, content, message.Body.CreateTime, message.Body.Hash, message.Body.SendRand)
	newmessage := NewMessage(head, body)
	newmessage.BuildReplyHash(message.Body.CreateTime, message.Body.Hash, message.Body.SendRand)
	// return session.Send(version_vnode_p2pHE, head.JSON(), body.JSON(), false)

	if this.vm.FindInVnodeSelf(*head.RecvVnode) {
		newmessage.Body.Content = content
		this.magneticSelf(newmessage)
		return nil
	}

	isOther, err := newmessage.Reply(version_vnode_p2pHE, this.nodeManager, this.sessionEngine, this.vm)
	if err != nil {
		return err
	}
	if !isOther {
		this.magneticSelf(newmessage)
	}
	return nil
}

/*
自己处理此方法
*/
func (this *MessageCenter) magneticSelf(message *Message) {
	// utils.Log.Info().Msgf("执行磁力到自己节点的方法:%d", message.Body.MessageId)
	//自己处理
	h := this.router.GetHandler(message.Body.MessageId)
	if h == nil {
		utils.Log.Info().Msgf("This broadcast message is not registered:", message.Body.MessageId)
		return
	}
	// utils.Log.Info().Msgf("有广播消息，消息编号 %d", message.Body.MessageId)
	//计算出落到自己的哪个虚拟节点上
	if message.Head.RecvVnode != nil && len(*message.Head.RecvVnode) != 0 {
		if message.checkRouterType() == version_vnode_p2pHE {
			targetVnodeID := this.vm.FindNearVnodeP2P(message.Head.RecvVnode, nil, true, nil)
			message.Head.SelfVnodeId = &targetVnodeID
		}
		if message.checkRouterType() == version_vnode_search {
			targetVnodeID := this.vm.FindNearVnodeSearchVnode(message.Head.RecvVnode, nil, true, false, nil)
			message.Head.SelfVnodeId = &targetVnodeID
		}
		if message.checkRouterType() == version_vnode_search_end {
			targetVnodeID := this.vm.FindNearVnodeSearchVnode(message.Head.RecvVnode, nil, true, true, nil)
			message.Head.SelfVnodeId = &targetVnodeID
		}
		// targetVnodeID := this.vm.FindNearVnode(message.Head.RecvVnode, nil, true)
		// message.Head.SelfVnodeId = &targetVnodeID
	}
	//TODO 这里获取不到控制器了,因此传空
	h(nil, engine.Packet{}, message)
}

/*
获取磁力节点地址
*/
func (this *MessageCenter) GetSearchNetAddr(nodeId, recvProxyId, senderProxyId *nodeStore.AddressNet, oneByOne bool, num uint16) ([]nodeStore.AddressNet, error) {
	var rbs []nodeStore.AddressNet
	var content []byte
	buf := bytes.NewBuffer(nil)
	err := binary.Write(buf, binary.LittleEndian, num)
	if err != nil {
		return nil, err
	}
	content = buf.Bytes()

	bs, err := this.SendSearchSuperMsgProxyWaitRequest(config.MSGID_searchID, nodeId, recvProxyId, senderProxyId, &content, time.Second*10, oneByOne)
	if err != nil || bs == nil {
		return nil, err
	}

	if oneByOne {
		vrp := new(go_protobuf.NodeAddrRepeated)
		err = proto.Unmarshal(*bs, vrp)
		if err != nil {
			utils.Log.Warn().Msgf("GetSearchNetAddr 不能解析proto:%s", err.Error())
			return nil, err
		}

		for i := range vrp.Nodes {
			rbs = append(rbs, nodeStore.AddressNet(vrp.Nodes[i].Nid))
		}
	} else {
		rbs = append(rbs, nodeStore.AddressNet(*bs))
	}

	return rbs, nil
}

/*
 * 发送一个新的查找超级节点消息，可以指定发送端的代理节点
 * @msgid			uint64		消息号
 * @recvid			addressnet	接收者节点id
 * @recvProxyId		addressnet	接收者代理节点id
 * @senderProxyId	addressnet	发送者代理节点id
 * @content			[]byte		发送内容
 * @return    bool    是否发送给其他节点:true=发送成功;false=发送失败;
 */
func (this *MessageCenter) SendSearchSuperMsgProxy(msgid uint64, recvid, recvProxyId, senderProxyId *nodeStore.AddressNet, content *[]byte) (*Message, error) {
	if !this.CheckOnline() {
		return nil, config.ERROR_offline
	}
	if recvid == nil || len(*recvid) == 0 {
		return nil, config.ERROR_params_fail
	}
	//搜索接口不能把消息发送给自己
	if bytes.Equal(this.nodeManager.NodeSelf.IdInfo.Id, *recvid) {
		return nil, config.ERROR_sent_myself
	}

	mhead := NewMessageHeadProxy(this.nodeManager.NodeSelf, this.nodeManager.GetSuperPeerId(), recvid, recvid, recvProxyId, senderProxyId, false, this.nodeManager.GetMachineID(), "")
	mbody := NewMessageBody(msgid, content, 0, nil, 0)
	message := NewMessage(mhead, mbody)
	message.BuildHash()
	isOther, err := message.Send(version_search_super, this.nodeManager, this.sessionEngine, this.vm, 0)
	if err != nil {
		return nil, err
	}
	if !isOther {
		this.magneticSelf(message)
	}
	return message, nil
}

/*
 * 发送一个新的查找超级节点消息，可以指定发送端的代理节点
 * @msgid			uint64		消息号
 * @recvid			addressnet	接收者节点id
 * @recvProxyId		addressnet	接收者代理节点id
 * @senderProxyId	addressnet	发送者代理节点id
 * @content			[]byte		发送内容
 * @return    bool    是否发送给其他节点:true=发送成功;false=发送失败;
 */
func (this *MessageCenter) SendSearchSuperMsgProxyWaitRequest(msgid uint64, recvid, recvProxyId, senderProxyId *nodeStore.AddressNet,
	content *[]byte, timeout time.Duration, oneByOne bool) (*[]byte, error) {
	if !this.CheckOnline() {
		return nil, config.ERROR_offline
	}
	if recvid == nil || len(*recvid) == 0 {
		return nil, config.ERROR_params_fail
	}
	mhead := NewMessageHeadProxy(this.nodeManager.NodeSelf, this.nodeManager.GetSuperPeerId(), recvid, recvid, recvProxyId, senderProxyId, false, this.nodeManager.GetMachineID(), "")
	mhead.OneByOne = oneByOne
	mbody := NewMessageBody(msgid, content, 0, nil, 0)
	message := NewMessage(mhead, mbody)

	message.BuildHash()

	flood.RegisterRequest(utils.Bytes2string(message.Body.Hash))
	defer flood.RemoveRequest(utils.Bytes2string(message.Body.Hash))
	isOther, err := message.Send(version_search_super, this.nodeManager, this.sessionEngine, this.vm, timeout)
	if err != nil {
		return nil, err
	}
	if !isOther {
		this.magneticSelf(message)
	}
	return flood.WaitResponse(utils.Bytes2string(message.Body.Hash), timeout)
}

/*
 * 给指定节点发送一个消息，可以指定发送端的代理节点
 *
 * @param	msgid			uint64		消息号
 * @param	recvid			addressnet	接收者节点id
 * @param	recvProxyId		addressnet	接收者代理节点id
 * @param	senderProxyId	addressnet	发送者代理节点id
 * @param	recvMachineId	string		接收方机器Id
 * @param	content			[]byte		发送内容
 * @return  msg				*Message    返回的消息
 * @return  sendSuccess		bool        是否发送成功
 * @return  toSelf			bool        消息是发给自己
 * @return  err				error       错误信息
 */
func (this *MessageCenter) SendP2pMsgProxy(msgid uint64, recvid, recvProxyId, senderProxyId *nodeStore.AddressNet, recvMachineId string, content *[]byte) (*Message, bool, bool, error) {
	if !this.CheckOnline() {
		return nil, false, false, config.ERROR_offline
	}
	if recvid == nil || len(*recvid) == 0 {
		return nil, false, false, config.ERROR_params_fail
	}

	// utils.Log.Info().Msgf("发送未加密消息 1111111111111 msgid:%d", msgid)
	head := NewMessageHeadProxy(this.nodeManager.NodeSelf, this.nodeManager.GetSuperPeerId(), recvid, recvid, recvProxyId, senderProxyId, true, this.nodeManager.GetMachineID(), recvMachineId)
	body := NewMessageBody(msgid, content, 0, nil, 0)
	message := NewMessage(head, body)
	message.BuildHash()

	if bytes.Equal(this.nodeManager.NodeSelf.IdInfo.Id, *recvid) {
		this.magneticSelf(message)
		return message, true, true, nil
	}

	isOther, err := message.Send(version_p2p, this.nodeManager, this.sessionEngine, this.vm, 0)
	if err != nil {
		return nil, false, !isOther, err
	}
	if !isOther {
		this.magneticSelf(message)
	}
	// utils.Log.Info().Msgf("发送未加密消息 2222222222222 msgid:%d ok:%t", msgid, ok)
	return message, true, !isOther, nil
}

/*
 * 给指定节点发送一个消息，可以指定接收端和发送端的代理节点
 *	@return    *[]byte      返回的内容
 *	@return    bool         是否发送成功
 *	@return    bool         消息是发给自己
 */
func (this *MessageCenter) SendP2pMsgProxyWaitRequest(msgid uint64, recvid, recvProxyId, senderProxyId *nodeStore.AddressNet, recvMachineId string,
	content *[]byte, timeout time.Duration) (*[]byte, bool, bool, error) {
	if !this.CheckOnline() {
		return nil, false, false, config.ERROR_offline
	}
	if recvid == nil || len(*recvid) == 0 {
		return nil, false, false, config.ERROR_params_fail
	}

	// var recvProxyID, senderProxyID string
	// if recvProxyId != nil && len(*recvProxyId) != 0 {
	// 	recvProxyID = recvProxyId.B58String()
	// }
	// if senderProxyId != nil && len(*senderProxyId) != 0 {
	// 	senderProxyID = senderProxyId.B58String()
	// }
	// utils.Log.Info().Msgf("发送未加密消息 msgid:%d, recvid:%s, senderid:%s, recvProxyId:%s, senderProxyId:%s", msgid, recvid.B58String(), this.nodeManager.NodeSelf.IdInfo.Id.B58String(), recvProxyID, senderProxyID)

	head := NewMessageHeadProxy(this.nodeManager.NodeSelf, this.nodeManager.GetSuperPeerId(), recvid, recvid, recvProxyId, senderProxyId, true, this.nodeManager.GetMachineID(), recvMachineId)
	body := NewMessageBody(msgid, content, 0, nil, 0)
	message := NewMessage(head, body)
	message.BuildHash()
	flood.RegisterRequest(utils.Bytes2string(message.Body.Hash))
	defer flood.RemoveRequest(utils.Bytes2string(message.Body.Hash))
	if bytes.Equal(this.nodeManager.NodeSelf.IdInfo.Id, *recvid) {
		this.magneticSelf(message)
		bs, err := flood.WaitResponse(utils.Bytes2string(message.Body.Hash), timeout)
		if err != nil {
			return nil, true, true, err
		}
		return bs, true, true, nil
	}
	isOther, err := message.Send(version_p2p, this.nodeManager, this.sessionEngine, this.vm, timeout)
	if err != nil {
		return nil, false, !isOther, err
	}
	if !isOther {
		this.magneticSelf(message)
	}
	bs, err := flood.WaitResponse(utils.Bytes2string(message.Body.Hash), timeout)
	if err != nil {
		return nil, true, !isOther, err
	}
	return bs, true, !isOther, nil
}

/*
 * 发送一个加密消息，包括消息头也加密，可以指定接收端和发送端的代理节点
 *	@return    *Message     返回的消息
 * 	@return    bool         是否发送成功
 *	@return    bool         消息是发给自己
 */
func (this *MessageCenter) SendP2pMsgHEProxy(msgid uint64, recvid, recvProxyId, senderProxyId *nodeStore.AddressNet, recvMachineId string, content *[]byte) (*Message, bool, bool, error) {
	if !this.CheckOnline() {
		return nil, false, false, config.ERROR_offline
	}
	if recvid == nil || len(*recvid) == 0 {
		return nil, false, false, config.ERROR_params_fail
	}
	if content == nil {
		bs := []byte{0}
		content = &bs
	}
	message, err := this.sendP2pMsgHELoadRatchet(msgid, recvid, recvProxyId, senderProxyId, recvMachineId, content)
	if err != nil {
		return nil, false, false, err
	}
	if bytes.Equal(this.nodeManager.NodeSelf.IdInfo.Id, *recvid) {
		message.Body.Content = content
		// utils.Log.Info().Msgf("是发送给自己:%d %s", len(*message.Body.Content), hex.EncodeToString(*message.Body.Content))
		this.magneticSelf(message)
		return message, true, true, nil
	}
	isOther, err := message.Send(version_p2pHE, this.nodeManager, this.sessionEngine, this.vm, 0)
	if err != nil {
		return nil, false, !isOther, err
	}
	if !isOther {
		this.magneticSelf(message)
	}
	// utils.Log.Info().Msgf("============ 发送加密消息 33333333333")
	return message, true, !isOther, nil
}

/*
 * 发送一个加密消息，包括消息头也加密，可以指定接收端和发送端的代理节点
 *	@return    *Message     返回的消息
 *	@return    bool         是否发送成功
 *	@return    bool         消息是发给自己
 */
func (this *MessageCenter) SendP2pMsgHEProxyWaitRequest(msgid uint64, recvid, recvProxyId, senderProxyId *nodeStore.AddressNet, recvMachineId string,
	content *[]byte, timeout time.Duration) (*[]byte, bool, bool, error) {
	if !this.CheckOnline() {
		return nil, false, false, config.ERROR_offline
	}
	if recvid == nil || len(*recvid) == 0 {
		return nil, false, false, config.ERROR_params_fail
	}
	if content == nil {
		bs := []byte{0}
		content = &bs
	}
	message, err := this.sendP2pMsgHELoadRatchet(msgid, recvid, recvProxyId, senderProxyId, recvMachineId, content)
	if err != nil {
		return nil, false, false, err
	}
	flood.RegisterRequest(utils.Bytes2string(message.Body.Hash))
	defer flood.RemoveRequest(utils.Bytes2string(message.Body.Hash))
	if bytes.Equal(this.nodeManager.NodeSelf.IdInfo.Id, *recvid) {
		message.Body.Content = content
		this.magneticSelf(message)
		bs, err := flood.WaitResponse(utils.Bytes2string(message.Body.Hash), timeout)
		return bs, true, true, err
	}

	isOther, err := message.Send(version_p2pHE, this.nodeManager, this.sessionEngine, this.vm, timeout)
	if err != nil {
		return nil, false, !isOther, err
	}
	if !isOther {
		this.magneticSelf(message)
	}
	bs, err := flood.WaitResponse(utils.Bytes2string(message.Body.Hash), timeout)
	// utils.Log.Info().Msgf("============ 发送加密消息 33333333333")
	return bs, true, !isOther, err
}

/*
 * 对某个消息回复，可以指定发送端的代理节点
 */
func (this *MessageCenter) SendSearchSuperReplyMsgProxy(message *Message, msgid uint64, content *[]byte, senderProxyId *nodeStore.AddressNet) error {
	return this.SendP2pReplyMsgProxy(message, msgid, content, senderProxyId)
}

/*
 * 对某个消息回复，可以指定发送端的代理节点
 */
func (this *MessageCenter) SendP2pReplyMsgProxy(message *Message, msgid uint64, content *[]byte, senderProxyId *nodeStore.AddressNet) error {
	if !this.CheckOnline() {
		return config.ERROR_offline
	}
	mhead := NewMessageHeadProxy(this.nodeManager.NodeSelf, this.nodeManager.GetSuperPeerId(), message.Head.Sender, message.Head.SenderSuperId, message.Head.SenderProxyId, senderProxyId, true, this.nodeManager.GetMachineID(), message.Head.SenderMachineID)
	mbody := NewMessageBody(msgid, content, message.Body.CreateTime, message.Body.Hash, message.Body.SendRand)
	newmessage := NewMessage(mhead, mbody)
	// message.BuildHash()
	newmessage.BuildReplyHash(message.Body.CreateTime, message.Body.Hash, message.Body.SendRand)
	isOther, err := newmessage.Reply(version_p2p, this.nodeManager, this.sessionEngine, this.vm)
	if err != nil {
		return err
	}
	if !isOther {
		this.magneticSelf(newmessage)
	}
	return nil
}

/*
 * 对某个消息回复，可以指定发送端的代理节点
 */
func (this *MessageCenter) SendP2pReplyMsgHEProxy(message *Message, msgid uint64, content *[]byte, senderProxyId *nodeStore.AddressNet) error {
	if !this.CheckOnline() {
		return config.ERROR_offline
	}
	mhead := NewMessageHeadProxy(this.nodeManager.NodeSelf, this.nodeManager.GetSuperPeerId(), message.Head.Sender, message.Head.SenderSuperId, message.Head.SenderProxyId, senderProxyId, true, this.nodeManager.GetMachineID(), message.Head.SenderMachineID)
	mbody := NewMessageBody(msgid, content, message.Body.CreateTime, message.Body.Hash, message.Body.SendRand)
	newmessage := NewMessage(mhead, mbody)
	// newmessage.BuildReplyHash(message.Body.CreateTime, message.Body.Hash, message.Body.SendRand)
	// if !newmessage.Reply(version_p2p, this.nodeManager, this.sessionEngine) {
	// 	this.magneticSelf(newmessage)
	// }

	isOther, err := newmessage.Reply(version_p2p, this.nodeManager, this.sessionEngine, this.vm)
	if err != nil {
		return err
	}
	if !isOther {
		this.magneticSelf(newmessage)
	}
	return nil
}

/*
 * 获取磁力节点地址，可以指定发送端的代理节点
 */
func (this *MessageCenter) GetSearchNetAddrProxy(nodeId, recvProxyId, senderProxyId *nodeStore.AddressNet) (*nodeStore.AddressNet, error) {
	bs, err := this.SendSearchSuperMsgProxyWaitRequest(config.MSGID_searchID, nodeId, recvProxyId, senderProxyId, nil, time.Second*10, false)
	if err != nil {
		return nil, err
	}
	netAddr := nodeStore.AddressNet(*bs)
	return &netAddr, nil
}
