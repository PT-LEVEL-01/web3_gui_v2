...
// 		utils.Log.Debug().Msgf("本机ip是公网全球唯一地址")
// 		return true
// 	}
// 	//尝试端口映射
// 	if portMapping() {
...
若支持upnp协议，则添加一个端口映射
*/
func portMapping(port uint16) bool {
	// utils.Log.Debug().Msgf("监听一个本地地址：%s:%d", Init_LocalIP, Init_LocalPort)
	...
	if err != nil {
		...
		utils.Log.Warn().Msgf("网关不支持端口映射", err)
		return false
	}

	//网关外网地址是不是全球唯一公网地址，不是则可能在双层网关内
	if !utils.IsOnlyIp(sys_mapping.GatewayOutsideIP) {
		utils.Log.Warn().Msgf("本机在双层网关内")
		return false
	}

	utils.Log.Debug().Msgf("正在尝试端口映射")
	for i := 0; i < 1000; i++ {
		// if err := sys_mapping.AddPortMapping(int(config.Init_LocalPort), int(config.Init_GatewayPort), "TCP"); err == nil {
		if err := sys_mapping.AddPortMapping(int(port), int(config.Init_GatewayPort), "TCP"); err == nil {
			config.Init_IsMapping = true
			utils.Log.Debug().Msgf("映射到公网地址：%s:%d", config.Init_GatewayAddress, config.Init_GatewayPort)
			return true
		}
		config.Init_GatewayPort = config.Init_GatewayPort + 1
	}
	utils.Log.Warn().Msgf("端口映射失败")
	// fmt.Println("端口映射失败")
	return false
}
