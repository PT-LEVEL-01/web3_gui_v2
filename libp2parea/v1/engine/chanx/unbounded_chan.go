...
// and out is used to read, which supports multiple readers.
// You can close the in channel if you want.
func NewUnboundedChan[T any](ctx context.Context, initCapacity int, maxQueue int) *UnboundedChan[T] {
	return NewUnboundedChanSize[T](ctx, initCapacity, initCapacity, initCapacity, maxQueue)
}
...
func NewUnboundedChanSize[T any](ctx context.Context, initInCapacity, initOutCapacity, initBufCapacity int, maxQueue int) *UnboundedChan[T] {
	...
	ch := UnboundedChan[T]{In: in, Out: out, buffer: NewRingBuffer[T](initBufCapacity, maxQueue)}
...
func process[T any](ctx context.Context, in, out chan T, ch *UnboundedChan[T]) {
	...
	for {
		...
		case val, ok := <-in:
			...
			for !ch.buffer.IsEmpty() && atomic.LoadInt64(&ch.bufCount) < int64(ch.buffer.maxQueue) {
				select {
				case <-ctx.Done():
					return
				case val, ok := <-in:
			...
			if atomic.LoadInt64(&ch.bufCount) >= int64(ch.buffer.maxQueue) {
				for !ch.buffer.IsEmpty() {
					select {
					case <-ctx.Done():
						return
					case out <- ch.buffer.Peek():
						ch.buffer.Pop()
						atomic.AddInt64(&ch.bufCount, -1)
						for len(in) > 0 && atomic.LoadInt64(&ch.bufCount) < int64(ch.buffer.maxQueue) {
							select {
							case val, ok := <-in:
								if !ok { // in is closed
									drain()
