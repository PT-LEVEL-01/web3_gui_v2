...
// 		if err != nil {
// 			// utils.Log.Warn().Msgf("网络错误 ServerConn %s", err.Error())
...
// 			// utils.Log.Debug().Msgf("server conn recv: %d %s %d", packet.MsgID, this.Ip, len(packet.Data)+16)

// 			// if packet.IsWait {
// 			// 	utils.Log.Debug().Msgf("开始等待")
// 			// 	packet.IsWait = false
// 			// 	packet.WaitChan <- true
// 			// 	utils.Log.Debug().Msgf("开始执行")
// 			// 	<-packet.WaitChan
// 			// 	utils.Log.Debug().Msgf("执行完成")
// 			// } else {
...
// 	} else {
// 		// utils.Log.Info().Msgf("server send %s", hex.EncodeToString(*buff))
