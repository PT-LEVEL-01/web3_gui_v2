...
func (this *ClientQuic) Connect(ip string, port uint32) (remoteName string, params interface{}, connectKey string, err error) {
	...
	if err != nil {
		utils.Log.Error().Msgf("connect error:%s ip:%s, port:%d", err.Error(), ip, port)
		return
	}
	stream, err := this.conn.OpenStreamSync(context.Background())
	if err != nil {
		// utils.Log.Error().Msgf("conn.OpenStreamSync:%s", err)
		return
	}
	this.stream = stream
...
func (this *ClientQuic) reConnect() {
	for {
		//十秒钟后重新连接
		...
		this.conn, err = quic.DialAddr(ctx, this.ip+":"+strconv.Itoa(int(this.port)), this.tlsConf, this.quicConf)
		if err != nil {
			cannel()
			continue
		}
