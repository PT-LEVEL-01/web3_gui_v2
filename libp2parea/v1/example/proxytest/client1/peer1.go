...
func (this *TestPeer) RecvP2PMsgHandler(c engine.Controller, msg engine.Packet, message *mc.Message) {
	...
	utils.Log.Info().Msgf("[%s] 收到p2p消息 self:%s sender:%s recvProxy:%s msgHash:%s", this.area.GetMachineID(), this.area.GetNetId().B58String(), message.Head.Sender.B58String(), recvProxy, msgHash)
