...
func StartAllPeer() {
	...
	targetID := nodeStore.AddressFromB58String("7AiN24imhsmeJNqg7EyfPQm9BEWx5M56Gf2o9r2Yz39i")
	// 测试磁力节点消息到达是否准确
	cnt := 0
	lastCnt := 0
	for {
		...
		if cnt > 0 && cnt%2 == 0 {
			utils.Log.Error().Msgf("qlw-----start change god addr information")
			port := 19960 + lastCnt%5
			success, godAddr := areas[0].SetAreaGodAddr("8.210.59.108", port)
			if success {
				curProxyId = godAddr
			}
			utils.Log.Error().Msgf("qlw-----end change god addr information!!!!")
			lastCnt++
			// if lastCnt%2 == 0 {
		...
		sendMsgOne(areas[0], &targetID)
		utils.Log.Info().Msgf("qlw----休息30秒后,将再次尝试获取目标地址")
...
func StartOnePeer(i int) *TestPeer {
	...
	// area.SetAreaGodAddr("127.0.0.1", 19963)
...
func sendMsgOne(area *libp2parea.Area, toAddr *nodeStore.AddressNet) error {
	//发送p2p消息
	index++
	content := []byte(toAddr.B58String() + "__|__" + strconv.Itoa(index) + "||" + curProxyId)
	utils.Log.Info().Msgf("节点:%s 发送消息给:%s, content:%s", area.GetNetId().B58String(), toAddr.B58String(), content)
	utils.Log.Info().Msgf("")
	var proxyId *nodeStore.AddressNet
	if curProxyId != "" {
		curProxyIdInfo := nodeStore.AddressFromB58String(curProxyId)
		proxyId = &curProxyIdInfo
		utils.Log.Warn().Msgf("qlw----proxyId:%s", curProxyId)
	} else {
		utils.Log.Warn().Msgf("qlw----don't have proxyId")
	}
	_, _, _, err := area.SendP2pMsgProxyWaitRequest(msg_id_p2p, toAddr, nil, proxyId, &content, time.Second*10)
	if err != nil {
