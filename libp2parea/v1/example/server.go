...
func StartAllPeer() {
	...
	// 		utils.Log.Info().Msgf("逻辑虚拟节点:%s %d %s", one.Nid.B58String(), one.Index, one.Vid.B58String())
	// 	}
	// }
...
func StartOnePeer(i int) *TestPeer {
	...
	area.StartUP(false, serverHost, uint16(basePort+i))
	// area.CloseVnode()
	// area.Vm.AddVnode()
	// area.Vm.AddVnode()
	// area.Vm.AddVnode()
