...
func StartAllPeer() {
	...
	// 	utils.Log.Info().Msgf("machineIDInfo-------------------------------start")
	// 	for ii := range logicNodes {
	// 		utils.Log.Info().Msgf("cur:%s curmachineID:%s target:%s tmachineID:%s", areaPeers[i].area.GetNetId().B58String(), areaPeers[i].area.GetMachineID(), logicNodes[ii].IdInfo.Id.B58String(), logicNodes[ii].MachineID)
	// 	}
	// 	utils.Log.Info().Msgf("machineIDInfo-------------------------------end")
	// }
