...
func StartOnePeer(i int) *TestPeer {
	...
	area.SetMachineID("area_machine_id_client1_" + strconv.Itoa(i))
...
func (this *TestPeer) RecvP2PMsgHandler(c engine.Controller, msg engine.Packet, message *mc.Message) {
	...
	var recvProxy, msgHash string
	if message.Head.RecvProxyId != nil {
		recvProxy = message.Head.RecvProxyId.B58String()
	}
	...
	utils.Log.Info().Msgf("[%s] 收到p2p消息 self:%s sender:%s recvProxy:%s content:%s msgHash:%s", this.area.GetMachineID(), this.area.GetNetId().B58String(), message.Head.Sender.B58String(), recvProxy, content, msgHash)
