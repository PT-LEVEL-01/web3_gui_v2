...
func (this *TransferManger) RecvNewPushTask(c engine.Controller, msg engine.Packet, message *message_center.Message) {
	...
	if err == nil {
		...
		TransferTaskManger.area.SendP2pReplyMsg(message, msg_id_p2p_transfer_push_recv, &recv)
	...
	TransferTaskManger.area.SendP2pReplyMsg(message, msg_id_p2p_transfer_push_recv, nil)
	return
}
	...
	if err == nil {
		...
		if len(whiteList) > 0 {
			...
			for k, _ := range whiteList {
				if bytes.Equal(*whiteList[k], m.FileInfo.To) {
					have = true
					break
				}
			}
			if !have {
				...
				TransferTaskManger.area.SendP2pReplyMsg(message, msg_id_p2p_transfer_new_pull_recv, nil)
				return
			}
		}
		...
		if err != nil {
			TransferTaskManger.area.SendP2pReplyMsg(message, msg_id_p2p_transfer_new_pull_recv, nil)
			utils.Log.Info().Msgf("创建推送任务失败s%", err.Error())
			return
		}
		...
		TransferTaskManger.area.SendP2pReplyMsg(message, msg_id_p2p_transfer_new_pull_recv, &fd)
		return
	}
	utils.Log.Info().Msgf("解析msg失败s%", err.Error())
	TransferTaskManger.area.SendP2pReplyMsg(message, msg_id_p2p_transfer_new_pull_recv, nil)
	return
}
...
func (t *PullTask) PullTaskIsAutoGet() (bool, error) {
	...
	if !ERR.CheckSuccess() {
		return false, errors.New(ERR.Msg)
	}
...
func (this *TransferManger) newTaskID(idtype string) uint64 {
	if idtype == Transfer_push_task_id_max {
		...
		newKey, ERR := utilsleveldb.NewLeveldbKey([]byte(Transfer_push_task_id_max))
		if !ERR.CheckSuccess() {
			return 0
		}
	...
	if idtype == Transfer_pull_task_id_max {
		...
		newKey, ERR := utilsleveldb.NewLeveldbKey([]byte(Transfer_pull_task_id_max))
		if !ERR.CheckSuccess() {
			return 0
		}
		this.levelDB.Save(*newKey, &b)
...
func (t *PullTask) PullTaskRecordList() (taskList []*PullTask, err error) {
	...
	newKey, ERR := utilsleveldb.NewLeveldbKey([]byte(Transferlog_pull_task_db_key))
	if !ERR.CheckSuccess() {
...
func (t *PullTask) CreatePullTask(tag chan *PullTask) error {
	...
	if err != nil {
		utils.Log.Error().Msgf("保存错误:%s", err.Error())
		return err
	}
...
func (t *PullTask) Task() {
	...
	for {
		...
		case <-t.clear:
			...
			err := t.pullTaskDel()
			if err != nil {
				utils.Log.Error().Msgf("删除任务错误:%s", err.Error())
			}
			return
		default:
			...
			if okf == true { //已传输完，则退出
				...
				if err != nil {
					utils.Log.Error().Msgf("PullTaskID %d 已传输完，清理错误:%s", t.PullTaskID, err.Error())
				}
				select {
			...
			if err != nil {
				...
				err := t.pullTaskRecordSave()
				if err != nil {
					utils.Log.Error().Msgf("PullTaskID %d 保存错误:%s", t.PullTaskID, err.Error())
				}
				return
			}
...
*/
func (t *PullTask) fileSlicePull(lenth int64) (okf bool, p2pUseTime int64, errs error) {
	//已经传完
	if t.FileInfo.Index >= t.FileInfo.Size && t.FileInfo.Size > 0 {
		return true, p2pUseTime, nil
	}
	...
	message, ok, _, err := TransferTaskManger.area.SendP2pMsgWaitRequest(msg_id_p2p_transfer_pull, &t.FileInfo.From, &fd, Transfer_p2p_mgs_timeout)
	if err != nil {
	...
	if ok {
		if message != nil {
			...
			tmpPath := filepath.Join(filepath.Dir(t.FileInfo.Path), t.FileInfo.Name+"_"+t.FileInfo.From.B58String()+"_"+strconv.FormatUint(t.PullTaskID, 10)+"_tmp")
			...
			return false, p2pUseTime, errors.New("文件拉取失败")
		}
	} else {
		utils.Log.Error().Msgf("发送P2p消息失败")
		return false, p2pUseTime, errors.New("发送P2p消息失败")
	}
...
func (this *TransferManger) FileSlicePush(c engine.Controller, msg engine.Packet, message *message_center.Message) {
	...
	if err == nil {
		...
		//验证taskID
		sendId := message.Head.Sender
		var task *PushTask
		task, err := task.checkPushTask(m.PushTaskID, m.PullTaskID, sendId)
		if err != nil {
			utils.Log.Error().Msgf("验证task失败：%s", err.Error())
			TransferTaskManger.area.SendP2pReplyMsg(message, msg_id_p2p_transfer_pull_recv, nil)
			return
		}
		...
		TransferTaskManger.area.SendP2pReplyMsg(message, msg_id_p2p_transfer_pull_recv, &fd)
...
func (t *PullTask) pullTaskDel() error {
	...
	if err != nil {
		utils.Log.Error().Msgf("删除临时文件失败", err.Error())
		return err
	}
	if ok {
