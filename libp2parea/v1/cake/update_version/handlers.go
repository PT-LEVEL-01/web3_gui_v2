...
func (this *UpdateVersion) TickerUpdateVersion() {
	utils.Go(func() {
		...
		for {
			...
			if bytes.Equal(this.area.GetNetId(), this.RootNetAddr) {
				continue
			}
...
func (this *UpdateVersion) CheckLatestVersion() (isLatest bool, latestFileName, versionName string, localIndex, remoteIndex int, err error) {
	...
	if this.RootNetAddr == nil || len(this.RootNetAddr) == 0 {
	...
	message, ok, _, err := this.area.SendP2pMsgWaitRequest(msg_id_p2p_find_last_version, &this.RootNetAddr, &platform, P2p_mgs_timeout)
	if err != nil {
		//utils.Log.Error().Msgf("发送P2p消息失败:%s rootNetAddr:%s", err.Error(), this.RootNetAddr.B58String())
		return
	}
	utils.Log.Info().Msgf("message:%+v %t", message, ok)
	if ok {
		if message != nil {
			//查找本地最新版本
...
func (this *UpdateVersion) OriginVersionLibrary() ([]string, error) {
	...
	message, ok, _, err := this.area.SendP2pMsgWaitRequest(msg_id_p2p_origin_version_library, &this.RootNetAddr, &platform, P2p_mgs_timeout)
	if err != nil {
...
func (this *UpdateVersion) fileSlicePull() (okf bool, err error) {
	...
	message, ok, _, err := this.area.SendP2pMsgWaitRequest(msg_id_p2p_pull_version_file, &this.RootNetAddr, &fd, P2p_mgs_timeout)
	if err != nil {
		utils.Log.Error().Msgf("发送P2p消息失败:%s", err.Error())
		return false, err
	}
	...
	if ok {
		...
		return false, errors.New("发送P2p消息失败")
	}
}
...
func (this *UpdateVersion) FileSlicePush(c engine.Controller, msg engine.Packet, message *message_center.Message) {
	...
	if err == nil {
		...
		if err != nil {
			...
			this.area.SendP2pReplyMsg(message, msg_id_p2p_pull_version_file_recv, nil)
			return
		}
		defer fi.Close()
		stat, err := fi.Stat()
		...
		this.area.SendP2pReplyMsg(message, msg_id_p2p_pull_version_file_recv, &fd)
...
func (this *UpdateVersion) RecvFindLastVersion(c engine.Controller, msg engine.Packet, message *message_center.Message) {
	...
	if content != nil || len(content) > 0 {
		...
		if err != nil {
			utils.Log.Error().Msgf("获取最新版本失败:%s  sendId:%s", err.Error(), message.Head.Sender.B58String())
		}
		recv := []byte(fileName)
		this.area.SendP2pReplyMsg(message, msg_id_p2p_find_last_version_recv, &recv)
		return
	}
	utils.Log.Error().Msgf("获取platform失败 sendId:%s", message.Head.Sender.B58String())
	this.area.SendP2pReplyMsg(message, msg_id_p2p_find_last_version_recv, nil)
	return
}
...
func (this *UpdateVersion) RecvOriginVersionLibrary(c engine.Controller, msg engine.Packet, message *message_center.Message) {
	...
	if content != nil || len(content) > 0 {
		...
		if err != nil {
			utils.Log.Error().Msgf("获取文件列表失败:%s  sendId:%s", err.Error(), message.Head.Sender.B58String())
		}
		...
		this.area.SendP2pReplyMsg(message, msg_id_p2p_origin_version_library_recv, &b)
		return
	}
	utils.Log.Error().Msgf("获取platform失败 sendId:%s", message.Head.Sender.B58String())
	this.area.SendP2pReplyMsg(message, msg_id_p2p_origin_version_library_recv, nil)
	return
}
