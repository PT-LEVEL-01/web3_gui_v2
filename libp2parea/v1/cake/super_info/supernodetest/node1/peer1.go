...
import (
	...
	superinfo "web3_gui/libp2parea/v1/cake/super_info"
...
func (this *TestPeer) SuperNodeOnlineCallbackFunc(addr nodeStore.AddressNet, machineID string) {
	utils.Log.Error().Msgf("qlw----- 超级节点上线回调 cur:%s, node:%s, machineID:%s", this.area.GetNetId().B58String(), addr.B58String(), machineID)
}
