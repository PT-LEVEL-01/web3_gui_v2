...
第二个块
*/
func BuildFirstBlock(area *libp2parea.Area) (*mining.BlockHeadVO, error) {
	Area = area
	//如果模块未启用，则不执行

	if !config.InitNode || !config.DB_is_null {
		// if !config.InitNode {
		// engine.Log.Info("不是创始节点")
		return nil, nil
	}
	...
	blockHead1 := mining.BlockHead{
		...
		Time:        config.TimeNow().Unix(),          //  time.Now().Unix(),                //unix时间戳
	...
	err := db.LevelDB.Save(bhashkey, bhbs)
	if err != nil {
...
func BuildReward(balanceTotal uint64) mining.TxItr {
	...
	for i, one := range reward.Vin {
		...
		if err != nil {
			utils.Log.Error().Err(err).Hex("puk", one.Puk).Send()
			return nil
		}
	...
	//		if !ok {
	//			utils.Log.Error().Str("未找到地址", key.GetAddrStr()).Send()
	...
	//			if err != nil {
	//				utils.Log.Error().Str("获取私钥错误", err.Error()).Send()
	//				return nil
	//			}
	//			sign := reward.GetSign(&prk, uint64(i))
...
func BuildDepositIn() mining.TxItr {
	...
	depositInBase := mining.TxBase{
		...
		LockHeight: 1,                  //锁定高度
		//		CreateTime: config.TimeNow().Unix(),                //创建时间
		Payload: config.Mining_first_witness_name,
		Comment: config.Mining_first_witness_name,
	}
	txin.TxBase = depositInBase
	txin.Rate = config.Mining_witness_ratio
	txin.BuildHash()
	return &txin
}
...
// 		NTx:     uint64(len(txHashes)),  //交易数量
// 		Tx:      txHashes,               //本区块包含的交易id
// 		Time:    config.TimeNow().Unix(),      //unix时间戳
...
// return pay, nil
func BuildRewardContractTx() mining.TxItr {
	baseCoinAddr := Area.Keystore.GetCoinbase()
	//构建输入
	...
	vins = append(vins, &vin)
	contractAddr := evmutils.AddressToAddressCoin(common.HexToAddress(config.RewardContractAddr).Bytes())
	engine.Log.Info("创世奖励合约地址%s", contractAddr.B58String())
	vouts := make([]*mining.Vout, 0)
	vouts = append(vouts, &mining.Vout{
		Address: contractAddr, //钱包地址
	})
	base := mining.TxBase{
		...
		Vout:       vouts,  //交易输出
		LockHeight: 1,      //
		Gas:        100000, //实际消耗50325
		Payload:    common.Hex2Bytes(precompiled.REWARD_RATE_BIN),
		//		CreateTime: config.TimeNow().Unix(),            //创建时间
		Comment: []byte{},
	}
	rewardContractTx := &mining.Tx_Contract{
		TxBase:   base,
		Action:   "create",
	...
	//给输出签名，防篡改
	for i, one := range rewardContractTx.Vin {
		for _, key := range Area.Keystore.GetAddrAll() {
			...
			if bytes.Equal(puk, one.Puk) {
				...
				sign := rewardContractTx.GetSign(&prk, uint64(i))
				//				sign := pay.GetVoutsSign(prk, uint64(i))
				rewardContractTx.Vin[i].Sign = *sign
			}
		}
	}
	rewardContractTx.BuildHash()
	return rewardContractTx
}

// 创建部署注册表合约交易
func BuildEnsContractTx() mining.TxItr {
	baseCoinAddr := Area.Keystore.GetCoinbase()
	//构建输入
	...
	vins = append(vins, &vin)
	contractAddr := evmutils.AddressToAddressCoin(common.HexToAddress(config.ENS_CONTRACT_ADDR).Bytes())
	engine.Log.Info("ens注册表合约地址%s", contractAddr.B58String())
	vouts := make([]*mining.Vout, 0)
	vouts = append(vouts, &mining.Vout{
		Address: contractAddr, //钱包地址
	})
	base := mining.TxBase{
		...
		LockHeight: 1,     //
		Gas:        50000, //
		Payload:    common.Hex2Bytes(ens.ENS_BIN),
		//		CreateTime: config.TimeNow().Unix(),            //创建时间
		Comment: []byte{},
	}
	rewardContractTx := &mining.Tx_Contract{
...
func BuildCloudStorageProxyContractTx() mining.TxItr {
	baseCoinAddr := Area.Keystore.GetCoinbase()
	//构建输入
	...
	vins = append(vins, &vin)
	engine.Log.Info("云储存代理合约地址%s", precompiled.CloudStorageProxyContract.B58String())
	vouts := make([]*mining.Vout, 0)
	vouts = append(vouts, &mining.Vout{
		Address: precompiled.CloudStorageProxyContract, //钱包地址
	})
	base := mining.TxBase{
		Type:       config.Wallet_tx_type_contract, //交易类型，默认0=挖矿所得，没有输入;1=普通转账到地址交易
		Vin_total:  1,
		...
		LockHeight: 1,      //
		Gas:        100000, //实际消耗50325
		Payload:    common.Hex2Bytes(cloud.CLOUD_PROXY_BIN),
		//		CreateTime: config.TimeNow().Unix(),            //创建时间
		Comment: []byte{},
	}
	rewardContractTx := &mining.Tx_Contract{
