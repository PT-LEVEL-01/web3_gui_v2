...
nft销毁
*/
type Tx_nft_destroy struct {
	TxBase
	NFT_ID []byte //NFT唯一id，转让的时候使用
}

type Tx_nft_destroy_VO struct {
	TxBaseVO
...
用于地址和txid格式化显示
*/
func (this *Tx_nft_destroy) GetVOJSON() interface{} {
	nft := Tx_nft_destroy_VO{
...
构建hash值得到交易id
*/
func (this *Tx_nft_destroy) BuildHash() {
	if this.Hash != nil && len(this.Hash) > 0 {
		return
	}
	bs := this.Serialize()
	id := make([]byte, 8)
	binary.PutUvarint(id, config.Wallet_tx_type_nft_destroy)
...
格式化成[]byte
*/
func (this *Tx_nft_destroy) Proto() (*[]byte, error) {
...
格式化成json字符串
*/
func (this *Tx_nft_destroy) Serialize() *[]byte {
...
获取签名
*/
func (this *Tx_nft_destroy) GetWaitSign(vinIndex uint64) *[]byte {
...
获取签名
*/
func (this *Tx_nft_destroy) GetSign(key *ed25519.PrivateKey, vinIndex uint64) *[]byte {
	signDst := this.GetSignSerialize(nil, vinIndex)
	*signDst = append(*signDst, this.NFT_ID...)
	// *signDst = append(*signDst, this.NFT_Owner...)
	// *signDst = append(*signDst, []byte(this.NFT_name)...)
...
检查交易是否合法
*/
func (this *Tx_nft_destroy) CheckSign() error {
...
/*
	验证是否合法
*/
// func (this *Tx_nft_destroy) GetWitness() *crypto.AddressCoin {
...
/*
	设置投票地址
*/
// func (this *Tx_nft_destroy) SetVoteAddr(addr crypto.AddressCoin) {
// 	// this.VoteAddr = addr
// 	// bs, err := this.Json()
// 	bs, err := this.Proto()
// 	if err != nil {
...
获取本交易总共花费的余额
*/
func (this *Tx_nft_destroy) GetSpend() uint64 {
	return this.Gas
}
...
检查重复的交易
*/
func (this *Tx_nft_destroy) CheckRepeatedTx(txs ...TxItr) bool {
	for _, one := range txs {
...
统计交易余额
*/
func (this *Tx_nft_destroy) CountTxItemsNew(height uint64) *TxItemCountMap {
...
func (this *Tx_nft_destroy) CountTxHistory(height uint64) {
	//转出历史记录
	hiOut := HistoryItem{
...
*/
func CreateTxNFTDestroy(gas uint64, pwd, comment string, nftid []byte) (*Tx_nft_destroy, error) {
	// engine.Log.Info("start CreateTxNFTDestroy")
	//查询这个NFT的所属者
	...
	var pay *Tx_nft_destroy
	for i := uint64(0); i < 10000; i++ {
		//没有输出
		base := TxBase{
			Type:       config.Wallet_tx_type_nft_destroy,               //交易类型
	...
	engine.Log.Info("end CreateTxNFTDestroy %s", hex.EncodeToString(*pay.GetHash()))
	return pay, nil
}
