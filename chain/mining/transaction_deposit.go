...
func (this *Tx_deposit_in) CheckSign() error {
	...
	if this.Vout[0].Value != config.Mining_deposit {
		return config.ERROR_deposit_witness
	}
	//判断vin是否太多
...
func (this *Tx_deposit_out) CheckSign() error {
	...
	//和自己有关的地址
	if this.Vout[0].CheckIsSelf() {
		item := GetLongChain().Balance.GetDepositIn()
		if item == nil {
			return config.ERROR_deposit_not_exist
		}
		if this.Vout[0].Value != item.Value {
			return config.ERROR_deposit_witness
		}
	}
...
@amount    uint64    押金额度
*/
func CreateTxDepositIn(amount, gas uint64, pwd, payload string, rate uint16) (*Tx_deposit_in, error) {
