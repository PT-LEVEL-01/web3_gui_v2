...
// var memUTXOTotal = int64(1)
// var dbUTXOTotal = int64(1)
...
// 		// engine.Log.Info("mem del utxo:%s_%d", hex.EncodeToString(one.Txid), one.VoutIndex)
// 		atomic.AddInt64(&memUTXOTotal, -1)
...
// 		// engine.Log.Info("mem add utxo:%s_%d frozenHeight:%d id:%d", hex.EncodeToString(item.Txid), item.VoutIndex, item.LockupHeight, memUTXOTotal)
// 		// atomic.AddInt64(&memUTXOTotal, 1)
// 		//根据冻结高度判断余额是否可用
...
// 	// engine.Log.Info("CountTxItem spend time: %s", config.TimeNow().Sub(start))
// }
...
// 		valueTotal += GetCommunityVoteRewardFrozen(&one.Addr)
// 	}
// 	frozen += valueTotal
// 	notspent -= valueTotal

// 	value := GetNotSpendBalance(&keystore.GetCoinbase().Addr)
// 	engine.Log.Info("旧余额统计:%d，新余额统计:%d", notspent+frozen+lock, value)
// 	this.lock.RUnlock()
// 	return
// }
...
// // 		this.Unfrozen(GetLongChain().GetCurrentBlock(), config.TimeNow().Unix())
// // 	}
// // }
...
// 	}
// 	this.lock.RUnlock()
// 	engine.Log.Info("Unfrozen spend time 222: %s", config.TimeNow().Sub(start))
// }
