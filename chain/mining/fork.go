...
// 	// engine.Log.Info("导入区块耗时 %s", config.TimeNow().Sub(start))
// }
...
func GetFirstBlock() error {
	...
	// engine.Log.Info("打印同步到的初始区块 %s", hex.EncodeToString(bhvo.BH.Hash))
...
func BuildFirstChain(bhvo *BlockHeadVO) {
	//初始化合约数量
	err := db.SetContractCount(big.NewInt(0))
	if err != nil {
		engine.Log.Info("初始化合约数量:%s", err.Error())
	}
...
创建一个起始空链,提供给快照
*/
func BuildSnapshotFirstChain() {
	newChain := NewChain()
	forks.LongChain = newChain

	startBlockHash, err := db.LevelDB.Get(config.Key_block_start)
	if err != nil {
		panic(err)
	}
	config.StartBlockHash = startBlockHash
...
创建一个起始空链,提供给轻节点快照
*/
func BuildSnapshotFirstLightChain() {
	newChain := NewChain()
	forks.LongChain = newChain
...
func (this *Forks) buildFirstChain(bhvo *BlockHeadVO) {
	if pre := crypto.ParseAddrPrefix(bhvo.BH.Witness); pre != config.AddrPre {
		engine.Log.Error("本节点地址前缀:%s 与创始节点地址前缀:%s 不一样！！", config.AddrPre, pre)
		panic("地址前缀\"" + config.AddrPre + "\"与创始节点地址前缀\"" + pre + "\"不一样！！")
	}
	...
	newChain.CurrentBlockHash = bhvo.BH.Hash

	// workModeLockStatic.GetImportBlockLock()
	// workModeLockStatic.BackImportBlockLockSuccess(bhvo.BH.GroupHeight, bhvo.BH.Height)
...
获取本节点同步高度
*/
func (this *Forks) GetCurrentHeight() uint64 {
	if this.LongChain == nil {
		return 0
	}
	return atomic.LoadUint64(&this.LongChain.CurrentBlock)
}
