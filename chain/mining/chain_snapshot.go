...
func InitChainSnap() {
	// 添加需要快照的对象
	// 主链Chain
	snapshot.Add(GetLongChain())
	// 快照起始高度
	snapshot.StartHeightAt()

	select {
	case onChainInitialized <- true:
	default:
	}
}

// 通过load命令加载快照
func StartChainSnapOfLoad() error {
	engine.Log.Info(">>>>>> Snapshot[加载] LOAD方式加载快照")
	BuildSnapshotFirstChain()

	// 初始化快照
	InitChainSnap()

	if err := snapshot.Load(); err != nil {
		engine.Log.Info(">>>>>> Snapshot[加载] 加载快照失败  %v", err)
		return err
	}

	AddWitnessAddrNets4Snapshot()

	// config.InitNode = true
	SetHighestBlock(db.GetHighstBlock())

	FindBlockHeight()

	if err := GetLongChain().LoadBlockChain(); err != nil {
		return err
	}
	FinishFirstLoadBlockChain()

	//初始化swap池
	GetLongChain().GetTransactionSwapManager().Initialize()
	return nil
}

// 通过正常启动方式，加载快照
func StartChainSnapOfNormal() error {
	engine.Log.Info(">>>>>> Snapshot[加载] 普通方式加载快照")
	BuildSnapshotFirstChain()
	...
	if err := GetLongChain().FirstDownloadBlock(); err != nil {
		return err
	}

	GetLongChain().NoticeLoadBlockForDB()
...
// 拉起快照后,添加一次见证人白名单
func AddWitnessAddrNets4Snapshot() {
	witAddrs := []*crypto.AddressCoin{}
	for _, wit := range GetLongChain().WitnessChain.witnessBackup.witnesses {
		witAddrs = append(witAddrs, wit.Addr)
	}
	go AddWitnessAddrNets(GetLongChain(), witAddrs)
}

// ////////////////////////////// 实现快照接口 ////////////////////////////////
func (this *Chain) SnapshotName() string {
	return "snapshot_chain"
}

// 序列化见证人链快照碎片
func (this *Chain) SnapshotSerialize() ([]byte, error) {
	b, err := this.snapshotSerialize()
	return b, err
}

// 链快照还原Chain
func (this *Chain) SnapshotDeSerialize(data []byte) error {
	startAt := config.TimeNow()
	defer engine.Log.Info(">>>>>> Snapshot[反序列化] 主链反序列化耗时(ms):%v,字节:%d", config.TimeNow().Sub(startAt), len(data))

	return this.snapshotDeSerialize(data)
}
...
func (this *Chain) snapshotSerialize() ([]byte, error) {
	...
	//// 未分配的见证人大组
	//var bigGroupNotGroup *go_protos.SnapshotWitnessBigGroup

	//// 未分配见证人集
	//for i, _ := range this.WitnessChain.witnessNotGroup {
	//	w1 := this.WitnessChain.witnessNotGroup[i]
	//	// 未分配见证人
	//	witnessNotGroups = append(witnessNotGroups, snapshotChain.toSnapshotWitness(w1))

	//	// 未分配见证人大组
	//	// 若是第60个大组的剩下的,则需要把见证人key放进来
	//	if bigGroupNotGroup == nil {
	//		bigGroupNotGroup = snapshotChain.toSnapshotWitnessBigGroup(w1.WitnessBigGroup)
	//	}

	//	if bigGroupNotGroup != nil {
	//		bigGroupNotGroup.Keys = append(bigGroupNotGroup.Keys, snapshotChain.witnessKey(w1))
	//	}

	//	// 若未分配的见证人和最后一个见证人是同一个大组，则添加一个keys的标记,使用时判断keys大于未分配见证人则说明有关系了
	//	if lastwitness.WitnessBigGroup == w1.WitnessBigGroup && i == 0 {
	//		bigGroupNotGroup.Keys = append(bigGroupNotGroup.Keys, snapshotChain.witnessKey(lastwitness))
	//	}

	//	// 所有未分配的见证人,备用见证人
	//	if _, ok := snapshotChain.WitnessChain.AllWitnessNotGroup[fmt.Sprintf("%p", w1)]; !ok {
	//		snapshotChain.WitnessChain.AllWitnessNotGroup[fmt.Sprintf("%p", w1)] = snapshotChain.toSnapshotWitness(w1)
	//	}
	//	for _, w2 := range w1.WitnessBigGroup.Witnesses {
	//		if _, ok := snapshotChain.WitnessChain.AllWitnessNotGroup[fmt.Sprintf("%p", w2)]; !ok {
	//			snapshotChain.WitnessChain.AllWitnessNotGroup[fmt.Sprintf("%p", w2)] = snapshotChain.toSnapshotWitness(w2)
	//		}
	//	}
	//	for _, w2 := range w1.WitnessBigGroup.WitnessBackup {
	//		if _, ok := snapshotChain.WitnessChain.AllWitnessNotGroup[fmt.Sprintf("%p", w2)]; !ok {
	//			snapshotChain.WitnessChain.AllWitnessNotGroup[fmt.Sprintf("%p", w2)] = snapshotChain.toSnapshotWitness(w2)
	//		}
	//	}
...
func (this *Chain) snapshotDeSerialize(data []byte) error {
	...
	// 当前的小组链
	// 清除小组的未统计的区块,避免导入区块时判断为重复导入
	if len(smallGroups[index].Witness) > 0 {
		for witness := smallGroups[index].Witness[0]; witness != nil; witness = witness.NextWitness {
			if witness != nil && witness.Block != nil && !witness.Block.isCount && witness.Block.Height > this.CurrentBlock {
				witness.Block = nil
			}
		}
	}
	...
	// 关联DB
	//engine.Log.Info(">>>>>> Snapshot 快照最新的block HASH %v, %v", blocks[0].Height, hex.EncodeToString(blocks[0].Id))
	//for _, v := range this.WitnessChain.witnessGroup.PreGroup.BlockGroup.Blocks {
	//	engine.Log.Info(">>>>>> Snapshot Chain最新的block HASH:%v,%v,%v", err, v.Height, hex.EncodeToString(v.Id))
	//}
	//height := this.CurrentBlock
	//hbash, err := db.LevelDB.Get([]byte(config.BlockHeight + strconv.Itoa(int(height))))
	//engine.Log.Info(">>>>>> Snapshot 存在块HASH:%v, %v, %v", err, height, hex.EncodeToString(hbash))

	//height = this.CurrentBlock + 1
	//hbash, err = db.LevelDB.Get([]byte(config.BlockHeight + strconv.Itoa(int(height))))
	//engine.Log.Info(">>>>>> Snapshot 存在块HASH:%v, %v, %v", err, height, hex.EncodeToString(hbash))
	//// db.LevelDB.Del([]byte(config.BlockHeight + strconv.Itoa(int(height))))

	//height = this.CurrentBlock + 2
	//hbash, err = db.LevelDB.Get([]byte(config.BlockHeight + strconv.Itoa(int(height))))
	//engine.Log.Info(">>>>>> Snapshot 存在块HASH:%v, %v, %v", err, height, hex.EncodeToString(hbash))
	//// db.LevelDB.Del([]byte(config.BlockHeight + strconv.Itoa(int(height))))

	//// 顶层元素对比
	//thiswit := this.WitnessChain.witnessGroup.Witness[len(this.WitnessChain.witnessGroup.Witness)-1]
	//thismockwit := this.WitnessChain.witnessGroup.Witness[len(this.WitnessChain.witnessGroup.Witness)-1]
	//// 大组
	//debug_biggroup := thiswit.WitnessBigGroup
	//mockdebug_biggroup := thiswit.WitnessBigGroup
	//// 小组
	//debug_smallgroup := this.WitnessChain.witnessGroup
	//mockdebug_smallgroup := this.WitnessChain.witnessGroup
	//// 块
	//debug_block := thiswit.Block
	//mockdebug_block := thiswit.Block
	//// 组
	//debug_group := thiswit.Group.BlockGroup
	//mockdebug_group := thiswit.Group.BlockGroup
	//// 见证人
	//debug_witness := thiswit
	//mockdebug_witness := thiswit

	//engine.Log.Info("%v", debug_biggroup, mockdebug_biggroup, debug_smallgroup, mockdebug_smallgroup, debug_block, mockdebug_block, debug_group, mockdebug_group, debug_witness, mockdebug_witness)

	//this = this
	return nil
}

func (c *Chain) SnapshotExtStorage() ([]db2.KVPair, error) {
	return nil, nil
}

func (s *Chain) SnapshotCallback(isSave bool) {
}
