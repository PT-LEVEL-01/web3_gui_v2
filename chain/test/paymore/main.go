...
func payMore() {
	...
	for i, _ := range payNumbers {
		...
		// one.Amount = one.Amount / 10
		payCount += one.Amount / 10
	}
	fmt.Println("地址个数:", len(payNumbers), "分发总量:", payCount)

	input := bufio.NewScanner(os.Stdin)
	fmt.Print("请输入支付密码:")
	input.Scan()
	pwd := input.Text()
	fmt.Println("您输入的密码是：", pwd)
	// return
	...
	for i := 0; i <= len(payNumbers)/pageNumber; i++ {
		...
		log.Println("开始分发，start:" + strconv.Itoa(start) + " end:" + strconv.Itoa(end))
		for _, one := range payNumbers[start:end] {
			log.Println("地址：", one.Address, "余额:", one.Amount)
		}
