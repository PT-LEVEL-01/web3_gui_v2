...
func main() {
	...
	fmt.Println("区块高度", 15190001, "奖励数量", config.ClacRewardForBlockHeightFun(15190002))
	// fmt.Println("区块高度", 100000000, "奖励数量", config.ClacRewardForBlockHeight(100000000))

	jiange := config.TimeNow().Sub(start)
	...
	total := uint64(0)
	for {
		reward := config.ClacRewardForBlockHeightFun(height)
		//整数亿块高度打印一次日志
		if height%3153600 == 0 {
