...
func init() {
	// tpc := new(payment.TokenPaymentController)
	// tpc.ActiveVoutIndex = new(sync.Map)
	// mining.RegisterTransaction(config.Wallet_tx_type_token_payment, tpc)
	// mining.RegisterTransaction(config.Wallet_tx_type_token_publish, new(publish.TokenPaymentController))
...
func start() {
	peer1 := Peer{
		Addr:       "127.0.0.1:3081",        //节点地址及端口，本地
	...
	//准备两个本地地址
	n, addrInfos := GetAddressList(peer1)
	if n < 2 {
		//不够2个，则创建
		peer1.CreatNewAddr()
	}
	fmt.Println("-----------------")
	//打印所有地址及余额
	_, addrInfos = GetAddressList(peer1)
	for _, one := range addrInfos {
		fmt.Println("地址:", one.AddrCoin, "余额:", one.Value)
	}
	fmt.Println("-----------------")
	//给第一个地址创建一个新的NFT
	firstAddr := addrInfos[0].AddrCoin
	peer1.CreatNFT(firstAddr)
	fmt.Println("-----------------")

	//等待上链
	fmt.Println("等待20秒上链 铸造NFT")
	time.Sleep(time.Second * 20)

	//查询第一个地址的NFT是否上链
	nftvos := peer1.FindNFTOne(firstAddr)
	if len(nftvos) == 0 {
		fmt.Println("未查询到NFT")
		return
	} else {
		fmt.Println("查询到", len(nftvos), "个NFT")
	}
	fmt.Println("-----------------")
	//把属于第一个地址的一个NFT赠送给第二个地址
	peer1.TransferNFT(nftvos[0].NFT_ID, addrInfos[1].AddrCoin)
	fmt.Println("-----------------")

	fmt.Println("等待20秒上链 转移NFT")
	time.Sleep(time.Second * 20)
	//查询所有NFT
	peer1.FindNFTAll()
	fmt.Println("-----------------")
	// peer1.TransferNFT(addrInfos[])
...
创建新NFT
*/
func (this *Peer) CreatNFT(owner string) bool {
	fmt.Println("创建新NFT")
	paramsChild := map[string]interface{}{
		"srcaddress": "",             //指定从哪个地址创建给手续费
		"gas":        1000000000,     //手续费
		"pwd":        this.WalletPwd, //支付密码
		"comment":    "",             //备注
		"owner":      owner,          //拥有者
		"name":       "nftname",      //nft的简称
		"symbol":     "nftsymbol",    //nft全称
		"resources":  "123.jpg",      //资源
	}
	params := map[string]interface{}{
		"method": "buildnft",  //rpc方法
		"params": paramsChild, //参数
	}
	result := Post(this.Addr, this.RPCUser, this.RPCPwd, params)
...
查询一个地址的NFT
*/
func (this *Peer) FindNFTOne(owner string) []*mining.Tx_nft_VO {
	fmt.Println("查询一个地址的NFT")
	paramsChild := map[string]interface{}{
		"address": owner,
	}
	params := map[string]interface{}{
		"method": "getnftbyowner", //rpc方法
		"params": paramsChild,     //参数
	}
	result := Post(this.Addr, this.RPCUser, this.RPCPwd, params)
	bs, err := json.Marshal(result.Result)
	...
	nftVO := make([]*mining.Tx_nft_VO, 0)
	err = json.Unmarshal(bs, &nftVO)
	if err != nil {
		fmt.Println("查询一个地址的NFT错误")
		panic(err.Error())
	}
	fmt.Println("查询一个地址的NFT完成")
	return nftVO
}
...
查询本钱包所有地址的NFT
*/
func (this *Peer) FindNFTAll() []*mining.Tx_nft_VO {
	fmt.Println("查询本钱包所有地址的NFT")
	paramsChild := map[string]interface{}{}
	params := map[string]interface{}{
		"method": "getnftall", //rpc方法
		"params": paramsChild, //参数
	}
	result := Post(this.Addr, this.RPCUser, this.RPCPwd, params)
	bs, err := json.Marshal(result.Result)
	...
	err = json.Unmarshal(bs, &nftVO)
	if err != nil {
		fmt.Println("查询所有NFT错误")
		panic(err.Error())
	}
	fmt.Println("查询本钱包所有地址的NFT完成")
	return nftVO
}
...
转让NFT
*/
func (this *Peer) TransferNFT(nftid string, owner string) bool {
	fmt.Println("转让NFT", nftid, owner)
	paramsChild := map[string]interface{}{
		"gas":     1000000000,     //手续费
		"pwd":     this.WalletPwd, //支付密码
		"comment": "",             //备注
		"nftid":   nftid,          //指定从哪个地址创建给手续费
		"owner":   owner,          //拥有者
	}
	params := map[string]interface{}{
		"method": "transfernft", //rpc方法
		"params": paramsChild,   //参数
	}
	result := Post(this.Addr, this.RPCUser, this.RPCPwd, params)
	_, err := json.Marshal(result.Result)
