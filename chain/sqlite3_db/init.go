...
var (
	...
	table_wallet_testdb *xorm.Session //测试表
)
...
func initWalletTable() {
	...
	engineDB.Table(TestDB{}).CreateTable(TestDB{}) //创建表格
	table_wallet_testdb = engineDB.Table(TestDB{}) //切换表格
	table_wallet_testdb.CreateIndexes(TestDB{})    //创建索引
	table_wallet_testdb.CreateUniques(TestDB{})    //创建唯一性约束
