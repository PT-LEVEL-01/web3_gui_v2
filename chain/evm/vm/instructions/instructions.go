...
func (i *instructionsContext) ExecuteContract(isCreate bool) ([]byte, uint64, []byte, []byte, error) {
	...
	i.pcCount = 0
	i.pc = 0
	//拿到合约对象，这里和以太坊有区别，以太坊是从新生成合约对象，利用state获取合约code,这里从上游传过来的
	...
	for {
		...
		//if i.environment.Contract.Address.B58String() != "MMS1111111111111111111DJ2Tn34" {
		//	fmt.Println("指令:", opcodes.OpCode(opCode))
		//}
	...
	i.timeUsed = config.TimeNow().Sub(startTime).Microseconds()
