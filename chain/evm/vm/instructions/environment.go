...
func addressAction(ctx *instructionsContext) ([]byte, error) {
	...
	address := evmutils.AddressCoinToAddress(ctx.environment.Contract.Address)
	addr.SetBytes(address.GetBytes())
...
func balanceAction(ctx *instructionsContext) ([]byte, error) {
	...
	balance, err := ctx.storage.Balance(toAddr)
	if err != nil {
		return nil, err
	}
...
func originAction(ctx *instructionsContext) ([]byte, error) {
	...
	address := evmutils.AddressCoinToAddress(ctx.environment.Transaction.Origin)
	origin.SetBytes(address.GetBytes())
	//ctx.stack.Push(ctx.environment.Transaction.Origin.Clone())
	ctx.stack.Push(origin)
	return nil, nil
}

func callerAction(ctx *instructionsContext) ([]byte, error) {
	caller := evmutils.New(0)
	address := evmutils.AddressCoinToAddress(ctx.environment.Message.Caller)
	caller.SetBytes(address.GetBytes())
	//ctx.stack.Push(ctx.environment.Message.Caller.Clone())
	ctx.stack.Push(caller)
	return nil, nil
}
...
func extCodeSizeAction(ctx *instructionsContext) ([]byte, error) {
	...
	//	engine.Log.Info("289行", addr.IsUint64(), addr.Uint64())
	//	addr.SetUint64(0)
	//	return nil, nil
	//}

	s, err := ctx.storage.GetCodeSize(toAddr)
	if err != nil {
		return nil, err
	}
	addr.Set(s.Int)
...
func extCodeCopyAction(ctx *instructionsContext) ([]byte, error) {
	...
	code, err := ctx.storage.GetCode(toAddr)
	if err != nil {
		return nil, err
	}
...
func extCodeHashAction(ctx *instructionsContext) ([]byte, error) {
	...
	codeHash, err := ctx.storage.GetCodeHash(toAddr)
	if err != nil {
		return nil, err
	}
...
func coinbaseAction(ctx *instructionsContext) ([]byte, error) {
	coinBase := evmutils.New(0)
	address := evmutils.AddressCoinToAddress(ctx.environment.Block.Coinbase)
	coinBase.SetBytes(address.GetBytes())
	//ctx.stack.Push(ctx.environment.Block.Coinbase.Clone())
	ctx.stack.Push(coinBase)
	return nil, nil
}
