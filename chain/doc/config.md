...
```text
  ...
  //是否开启自重启功能
  "EnableRestart": false
  //最低cpu核数,仅在启动时检查,0=不检查,大于0就按给定值进行检查,不符合就终止程序
  "MinCPUCores": 0, 
  //最低可用内存(MB),仅在启动时检查,0=不检查,大于0就按给定值进行检查,不符合就终止程序
  "MinFreeMemory": 0,
  //最低可用磁盘空间(MB),仅在启动时检查,0=不检查,大于0就按给定值进行检查,不符合就终止程序
  "MinFreeDisk": 0
  //最低带宽(MB/s),仅在启动时检查,0=不检查,大于0就按给定值进行检查,不符合发出警告
