...
func (a *SdkApi) IM_GetSelfInfo() map[string]interface{} {
	resultMap := make(map[string]interface{})
	userInfo, ERR := db.GetSelfInfo(im.Area.GetNetId())
	if !ERR.CheckSuccess() {
		resultMap["code"] = ERR.Code
...
func (a *SdkApi) IM_SetSelfInfo(nickname string, headNum uint64, traySwitch bool) map[string]interface{} {
	...
	ERR = db.UpdateSelfInfo(im.Area.GetNetId(), userInfo)
	if !ERR.CheckSuccess() {
...
func (a *SdkApi) IM_GetFriendInfo(addrStr string) map[string]interface{} {
	...
	addr := nodeStore.AddressFromB58String(addrStr)
	userinfo, ERR := db.FindUserListByAddr(im.Area.GetNetId(), addr)
	...
	if userinfo != nil {
		...
		if userinfo.Nickname == "" {
			ui, ERR := db.ImProxyClient_FindUserinfo(im.Area.GetNetId(), addr)
	...
	go func(have bool) {
		...
		//修改好友信息
		ERR = db.ImProxyClient_SaveUserinfo(im.Area.GetNetId(), userInfo)
		if !ERR.CheckSuccess() {
