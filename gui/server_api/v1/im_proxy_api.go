...
func (a *SdkApi) IMProxyServer_GetProxyInfo() map[string]interface{} {
	...
	info, ERR := db.ImProxyServer_GetProxyInfoSelf(im.Area.GetNetId())
	if ERR.CheckFail() {
	...
	if info == nil {
		...
		ERR := db.ImProxyServer_SetProxyInfoSelf(im.Area.GetNetId(), info)
		if !ERR.CheckSuccess() {
			//ERR := utils.NewErrorSysSelf(err)
			resultMap["code"] = ERR.Code
			resultMap["error"] = ERR.Msg
...
func (a *SdkApi) IMProxyServer_SetOpen(isOpen bool) map[string]interface{} {
	ERR := utils.NewErrorSuccess()
	resultMap := make(map[string]interface{})
	sinfo, ERR := db.ImProxyServer_GetProxyInfoSelf(im.Area.GetNetId())
	if ERR.CheckFail() {
		//ERR = utils.NewErrorSysSelf(err)
		resultMap["code"] = ERR.Code
		resultMap["error"] = ERR.Msg
	...
	ERR = db.ImProxyServer_SetProxyInfoSelf(im.Area.GetNetId(), sinfo)
	if !ERR.CheckSuccess() {
		//ERR = utils.NewErrorSysSelf(err)
		resultMap["code"] = ERR.Code
		resultMap["error"] = ERR.Msg
...
func (a *SdkApi) IMProxyClient_GetProxyList() map[string]interface{} {
	resultMap := make(map[string]interface{})
	imServers, ERR := db.ImProxyClient_GetProxyList(im.Area.GetNetId())
	if ERR.CheckFail() {
...
获取订单列表
*/
func (a *SdkApi) ImProxyClient_GetOrderList() map[string]interface{} {
	...
	////	return resultMap
	////}
	//proxyVOs := make([]*model.StorageServerInfoVO, 0, len(orders))
