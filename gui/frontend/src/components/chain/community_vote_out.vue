<template>
<div>
      <el-page-header @back="goBack" content="取消社区质押">
    </el-page-header>
...
const ruleForm = ref({
  witnessAddr: store.witnessScoreOut.WitnessAddr,
  witnessName: store.witnessScoreOut.Payload,
  amount:store.witnessScoreOut.Value,
  addr:store.witnessScoreOut.AddrSelf,
  gas:"",
  comment:"",
  pass: ''
})
...
function submitForm(formName) {
  thistemp.$refs[formName].validate((valid) => {
    ...
    Promise.all([Chain_VoteOut(1,ruleForm.value.addr,amount,gas,0,ruleForm.value.pass,
        ruleForm.value.comment)]).then(messages => {
      if(!messages || !messages[0]){return}
      var messageOne = messages[0];
      console.log("取消社区质押结果",messageOne)
    ...
    // params.data = {method:"voteout",params:{witness:ruleForm.value.witnessAddr,address:ruleForm.value.addr,txid:store.witnessScoreOut.Txid,amount:amount,gas:gas,pwd:ruleForm.value.pass}}
    // console.log(params);
