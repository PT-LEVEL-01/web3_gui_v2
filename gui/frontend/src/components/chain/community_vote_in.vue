<template>
<div>
    <el-page-header @back="goBack" content="给社区节点投票">
    </el-page-header>
...
import { Chain_GetVoteList, Chain_GetCoinAddress, Chain_VoteIn } from '../../../bindings/web3_gui/gui/server_api/sdkapi'
...
function submitForm(formName) {
  thistemp.$refs[formName].validate((valid) => {
    ...
    Promise.all([Chain_VoteIn(2,ruleForm.value.addr,ruleForm.value.witnessAddr,0,amount,gas,0,
        ruleForm.value.pass,ruleForm.value.comment)]).then(messages => {
      if(!messages || !messages[0]){return}
      var messageOne = messages[0];
      console.log("投票返回",messageOne)
    ...
    // params.data = {method:"votein",params:{votetype:2,address:ruleForm.value.addr, witness:ruleForm.value.witnessAddr,amount:amount,gas:gas,pwd:ruleForm.value.pass,payload:ruleForm.value.comment}};
...
onUnmounted(() => {
  store.setCommunityVote(null);
});


function created(){
  // console.log("选中的社区",store.communityVote)
  Promise.all([Chain_GetVoteList(2), Chain_GetCoinAddress()]).then(messages => {
    if(!messages || !messages[0]){return}
    var communityVoteList = messages[0];
    console.log("获取的投票列表",communityVoteList)
    ...
    for(var j = 0,len=messageOne.length; j < len; j++) {
      ...
      if(one.Type == 1){
        ...
        one.AddrCoin = one.AddrCoin+" 普通节点"
        one.disable = true
      }
  ...
  //     return
  //   }
  //   var communityVoteList = response.data.result;
  //   console.log(communityVoteList)
  //
  //   //获取所有地址列表
  //   var params = JSON.parse(JSON.stringify(store.rpcParams));
  ...
  //         //判断这个情节点地址是否已经投票了
  //         var have = false;
  //         for(var i=0; i<communityVoteList.length; i++){
  //           if(communityVoteList[i].AddrSelf == one.AddrCoin && communityVoteList[i].WitnessAddr != store.communityVote.Addr){
  //             have = true;
  //             break;
  //           }
  //         }
  //         if(have){continue}
  //         if(ruleForm.value.addrs.length == 0){
