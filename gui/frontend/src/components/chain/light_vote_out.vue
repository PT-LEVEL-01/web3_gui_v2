<template>
<div>
      <el-page-header @back="goBack" content="取消投票">
    </el-page-header>
...
<el-form :model="ruleForm" status-icon :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm"  style="margin-top:40px;">
  ...
  <el-form-item label="社区节点地址" prop="witnessAddr">
...
const ruleForm = ref({
  witnessAddr: store.lightVoteOut.WitnessAddr,
  witnessName: store.lightVoteOut.Payload,
  amount:store.lightVoteOut.Value,
  addr:store.lightVoteOut.AddrSelf,
  gas:"",
  comment:"",
  pass: ''
})
...
function submitForm(formName) {
  thistemp.$refs[formName].validate((valid) => {
    ...
    // Promise.all([Chain_VoteOut(2,ruleForm.value.addr,amount,gas,0,ruleForm.value.pass,ruleForm.value.comment)]).then(messages => {
    //   if(!messages || !messages[0]){return}
    //   var messageOne = messages[0];
    //   console.log("取消投票结果",messageOne)
    ...
    // params.data = {method:"voteout",params:{witness:ruleForm.value.witnessAddr,address:ruleForm.value.addr,txid:store.lightVoteOut.Txid,amount:amount,gas:gas,pwd:ruleForm.value.pass}}
    // console.log(params);
