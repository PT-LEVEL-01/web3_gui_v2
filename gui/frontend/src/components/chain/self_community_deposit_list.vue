<template>
  <div class="about">
        <el-page-header @back="goBack" content="我的社区质押记录">
        </el-page-header>
...
function vote(index, rows) {
  // rows.splice(index, 1);
  store.setWitnessScoreOut(rows[index]);
  thistemp.$router.push({path: '/index/wallet/communityvoteout'});
}
...
function created(){
  Promise.all([Chain_GetVoteList(1)]).then(messages => {
    if(!messages || !messages[0]){return}
    var messageOne = messages[0];
    console.log("我的质押记录列表",messageOne)
  ...
  // params.data = {method:"getvotelist",params:{votetype:1}};
