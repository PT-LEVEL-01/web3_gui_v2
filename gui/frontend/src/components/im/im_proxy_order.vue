<template>
  <div style="text-align: left;border:red solid 0px;">
    <el-page-header @back="back" content="订单">
    </el-page-header>
...
import { ImProxyClient_GetOrder } from '../../../bindings/web3_gui/gui/server_api/sdkapi'
...
const createOrder = () => {
  Promise.all([ImProxyClient_GetOrder(storageServerInfo.value.Addr, parseInt(OrderSize.value), parseInt(OrderTime.value))]).then(messages => {
    if(!messages || !messages[0]){return}
    var messageOne = messages[0];
    var result = thistemp.$checkResultCode(messageOne.code)
    console.log("返回订单结果",messageOne)
    if(!result.success){
      ElMessage({
        showClose: true,
