...
<script setup>
import { Storage_Client_uploadList } from '../../../bindings/web3_gui/gui/server_api/sdkapi'
...
const percentage = (row) => {
  return Math.floor(row.OffsetIndex/row.FileSize*100)
}

//处理文件传输列表进度刷新
const flashUploadList = () => {
  Promise.all([Storage_Client_uploadList()]).then(messages => {
    ...
    console.log("上传列表",messageOne.list)
