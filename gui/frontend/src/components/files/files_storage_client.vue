...
import { Storage_client_GetStorageServiceList, Storage_client_GetSearchStorageServiceList } from '../../../bindings/web3_gui/gui/server_api/sdkapi'
...
const storageOrderList = () => {
  thistemp.$router.push({path: '/index/files/orders'});
}
//显示购买空间页面
...
const storageServerSearchPage = () => {
  thistemp.$router.push({path: '/index/files/storage_client_search'});
}
...
const getStorageList = () => {
  Promise.all([Storage_client_GetStorageServiceList()]).then(messages => {
    if(!messages || !messages[0]){return}
    var messageOne = messages[0];
    var result = thistemp.$checkResultCode(messageOne.code)
    if(!result.success){
