...
func init() {
	RegisterCmdClient(config.IMPROXY_Command_client_group_sendText, ParseDataChainGroupSendTextFactory)
}

/*
发送文本消息
*/
type DataChainGroupSendText struct {
	DataChainClientBase
	//Type          uint64               //消息类型
...
func (this *DataChainGroupSendText) CheckCmd() bool {
	return true
}

func (this *DataChainGroupSendText) Proto() (*[]byte, error) {
...
初始化指令中必须包含完整的好友列表
*/
func ParseDataChainGroupSendTextFactory(bs []byte) (DataChainClientItr, error) {
	...
	sendText := DataChainGroupSendText{
...
func NewDataChainGroupSendText(addrSelf nodeStore.AddressNet, groupId []byte, content, quotoId []byte) *DataChainGroupSendText {
	forward := NewDataChainProxyForward(addrSelf, nodeStore.AddressNet{})
	forward.GroupID = groupId
	clientBase := NewDataChainClientBase(config.IMPROXY_Command_client_group_sendText, addrSelf, nodeStore.AddressNet{})
	sendText := DataChainGroupSendText{DataChainClientBase: *clientBase}
