...
type KeystoreInterface interface {
	...
	GetFilePath() string
	GetNewAddrByName(name, password, newAddressPassword string) (crypto.AddressCoin, error)
}
...
创建一个新的24位助记词种子文件
*/
func (this *Keystore) CreateNewKeystoreBy32(password string) error {
	pwd := sha256.Sum256([]byte(password))

	seed, err := crypto.Rand32Byte() //随机生成32byte
	if err != nil {
		return err
	}
	seedBs := seed[:]
