...
func (this *Keystore) Load() error {
	...
	//utils.Log.Info().Str("密钥文件内容", string(bs)).Send()
	...
	if err != nil {
		if err = this.oldKeystoreLoad(bs); err != nil {
			//utils.Log.Info().Str("密钥文件内容", err.Error()).Send()
			return err
		}
		//utils.Log.Info().Str("密钥文件内容", "11111").Send()
		return nil
	}
	//utils.Log.Info().Str("密钥文件内容", "11111").Send()
	this.lock = new(sync.RWMutex)
...
func (this *Keystore) GetNewAddr(password, newAddressPassword string) (crypto.AddressCoin, error) {
	...
	//utils.Log.Info().Msgf("生成参数1:%x %x %x", this.Seed, newAddressPassword, Salt)
