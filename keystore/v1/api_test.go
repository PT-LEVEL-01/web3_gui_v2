...
func InitKeystore() {
	derivation.CoinType = 123456789
	//path := "keystore.key"
	path := "D:\\test\\temp/keystore.key"
	addrPre := "TEST"
	pwd := "123456789"
	...
	if keystoreStatic.NetAddr == nil {
		_, _, err = keystoreStatic.CreateNetAddr(pwd, pwd)
		if err != nil {
	...
	if len(GetAddrAll()) == 0 {
		for i := range 5 {
			addr, err := GetNewAddr(pwd, pwd)
			if err != nil {
				fmt.Println("GetNewAddr error:", err.Error())
				return
			}
			fmt.Println("生成的地址", i, addr.B58String())
		}
	}

	if len(GetDHKeyPair().SubKey) == 0 {
		_, err := keystoreStatic.GetNewDHKey(pwd, pwd)
		if err != nil {
			fmt.Println("GetNewDHKey error:", err.Error())
			return
		}
...
func useKeyStore() {
	...
	ok, err := UpdatePwd(pwd, newAddrPwd)
	fmt.Println("UpdatePwd:", ok, " err:", err)
	ok, err = UpdateNetAddrPwd(newAddrPwd, pwd)
