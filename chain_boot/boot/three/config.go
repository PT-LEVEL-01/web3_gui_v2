...
const (
	FILEPATH_keystore_old = "conf/keystore.key"
)
...
*/
func InitConfig() {
	//derivation.CoinType = 0x800055ab                    //
	//kv2config.CoinType_coin = 0x800055ab                //
	//kv2config.CoinType_dh = kv2config.CoinType_coin + 1 //
	config.AddrPre = "THREE"                               //
	derivation.CoinType = 0x800055ab                       //
	keystore.SetCoinTypeCustom(0x800055ab, config.AddrPre) //
	keystore.SetCoinTypeCustomDH(0x800055ab + 1)           //
	kconfig.ShortAddress = true                            //使用短地址

	gconfig.AreaName_str = "three_net"                                              //
	config.Path_config = "config_v2.json"                                           //配置文件名称
	...
	config.Mining_block_time = 5 * time.Second       //出块时间，单位：秒
	config.Mining_coin_total = 3690 * 10000 * 1e8    //货币发行总量3690万
	config.Mining_coin_premining = 100 * 10000 * 1e8 //预挖量 100万
	config.Mining_vote = uint64(10000 * 1e8)         //社区节点投票押金最少金额
	config.Mining_light_min = uint64(10 * 1e8)       //轻节点押金最少金额
