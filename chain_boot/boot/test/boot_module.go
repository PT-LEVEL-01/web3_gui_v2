...
import (
	...
	libp2pv2 "web3_gui/libp2parea/v2"
	"web3_gui/utils"
)
...
区块链服务启动
*/
func StartChainWithNode(node *libp2pv2.Node, password, configJSONStr string, init bool) error {
	//InitConfig()
	area := libp2parea.BuildArea(node, password)
	config.Area = area
...
func register(area *libp2parea.Area) error {
	...
	if config.InitNode {
		os.RemoveAll(config.DB_path)
	}
	//检查目录是否存在，不存在则创建
	utils.CheckCreateDir(config.DB_path)
	...
	if err := hardware.CheckHardwareRequirement(config.MinCPUCores, config.MinFreeMemory, config.MinFreeDisk,
		config.MinNetworkBandwidth, peerIps); err != nil {
	...
	if config.InitNode {
		...
		//utils.Log.Info().Msgf("2222222222222")
		// 初始化快照
...
循环从邻居节点获取起始区块
*/
func LoopGetFirstBlock() error {
	var err error
	for {
		//从邻居节点同步区块
		err = mining.GetFirstBlock()
		if err == nil {
			//utils.Log.Error().Str("获取首块为空", "").Send()
			return nil
		}
		utils.Log.Error().Str("err", err.Error()).Send()
		utils.Log.Info().Str("从邻居节点同步区块失败", "稍后自动重试").Send()
		time.Sleep(time.Minute)
	}
}
